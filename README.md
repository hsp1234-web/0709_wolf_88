...
本專案的核心圍繞一個異步任務管線，您可以透過 `run.py` 進行所有操作。
...
1.  **(可選) 清理舊數據:**
    ```bash
    # 清除所有先前的回測結果，以便重新開始
    poetry run python run.py clear-results
    ```
...
3.  **派發計算任務:**
    * 在**另一個**終端機中，向佇列新增一批計算任務。
    ```bash
    # 新增一批包含動態參數的SMA交叉策略計算任務
    poetry run python run.py add-test-tasks
    ```

4.  **啟動儀表板以可視化結果:**
    * 計算完成後，啟動Web服務。
    ```bash
    # 啟動儀表板，預設位於 http://127.0.0.1:8000
...
### **5.2 (歷史) 主要功能執行 (v0.6.0 新版 CLI 用法)**
