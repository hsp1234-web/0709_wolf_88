...
完成上述步驟後，您的本地環境即已準備就緒，可以開始執行各個微應用或進行測試。

**重要更新：【蒼穹之心計畫】**

隨著【蒼穹之心計畫】的推進，我們正在將各個獨立的應用程式整合到一個統一的**中央指揮部 (CLI)**。這個中央指揮部由位於專案根目錄的 `main.py` 腳本提供。您可以使用它來訪問已整合的應用功能。

### **5.5、 中央指揮部：啟動應用功能**

所有已整合到中央指揮部的功能都可以通過 `python main.py [COMMAND]` 的形式調用。您可以通過以下指令查看所有可用的命令：
...
*   **執行回測引擎 (Backtesting Engine):**
    要執行回測引擎的核心邏輯，請使用 `backtest` 命令：
    ```bash
