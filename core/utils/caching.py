...
if __name__ == '__main__':
    ...
    end_time = time.time()
    print(f"第一次請求完成。耗時: {end_time - start_time:.2f} 秒。From Cache: {response1.from_cache}")

    print("\n正在進行第二次請求 (應立即完成)...")
    start_time = time.time()
    response2 = session.get(test_url)
    end_time = time.time()
    print(f"第二次請求完成。耗時: {end_time - start_time:.2f} 秒。From Cache: {response2.from_cache}")
    ...
    end_time = time.time()
    print(f"強制刷新請求完成。耗時: {end_time - start_time:.2f} 秒。From Cache: {response3.from_cache}")
