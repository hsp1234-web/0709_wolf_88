...
class DBManager:
    ...
    def _connect(self):
        ...
        try:
            ...
            logger.error(f"連接到 DuckDB 數據庫 {self.db_path} 失敗: {e}", exc_info=True)
            raise
        ...
        try:
            ...
            if if_exists == 'replace':
                self.connection.execute(f"CREATE OR REPLACE TABLE {table_name} AS SELECT * FROM {temp_view_name}")
                 ...
                 self.connection.execute(f"INSERT INTO {table_name} SELECT * FROM {temp_view_name}")
            else: # 'fail'
                self.connection.execute(f"CREATE TABLE {table_name} AS SELECT * FROM {temp_view_name}")

            logger.info(f"成功將 {len(df)} 筆記錄寫入到數據表 '{table_name}' (模式: {if_exists})。")
            ...
            if create_index and df.index.name:
                ...
                logger.info(f"在數據表 '{table_name}' 的 '{index_name}' 欄位上創建索引。")
