...
class StressIndexCalculator:
    ...
    def _fetch_all_data(self) -> Dict[str, pd.DataFrame]:
        ...
        # 獲取紐約聯儲數據
        try:
            nyfed_df = self.nyfed_client.fetch_data(symbol="DUMMY_NYFED_SYMBOL_IGNORED") # Symbol is ignored by NYFedClient
            if not nyfed_df.empty:
                if 'Date' in nyfed_df.columns and 'Total_Positions' in nyfed_df.columns:
                    ...
                    data_frames['NYFed_Positions'] = nyfed_df[['Total_Positions']].rename(columns={'Total_Positions': 'NYFed_Positions'})
                    print(f"資訊：NYFed 數據獲取成功，共 {len(data_frames['NYFed_Positions'])} 筆。")
                else:
                    print(f"錯誤：NYFed 數據缺少 'Date' 或 'Total_Positions' 欄位。可用欄位: {nyfed_df.columns.tolist()}")
                    data_frames['NYFed_Positions'] = pd.DataFrame(columns=['NYFed_Positions']).set_index(pd.to_datetime([]))
            else:
        ...
        for name, df_item in data_frames.items():
            ...
            if not df_item.empty:
                if not isinstance(df_item.index, pd.DatetimeIndex):
                    ...
                    print(f"警告：FRED 數據框 '{name}' 的索引不是 DatetimeIndex。將嘗試轉換。")
                    ...
                    elif df_item.index.name == 'Date' and not isinstance(df_item.index, pd.DatetimeIndex):
                        df_item.index = pd.to_datetime(df_item.index)
                    else:
                        print(f"錯誤：FRED 數據框 '{name}' 索引轉換失敗。")
                        data_frames[name] = pd.DataFrame(columns=[name]).set_index(pd.to_datetime([])) # Empty df with correct name
                ...
                if name in df_item.columns:
                     data_frames[name] = df_item[[name]] # Select only the relevant column
                elif len(df_item.columns) == 1: # If only one column, assume it's the correct one
                    df_item.columns = [name]
                    data_frames[name] = df_item
                else:
                    print(f"警告：FRED 數據框 '{name}' 的欄位與預期不符: {df_item.columns.tolist()}。將嘗試使用第一個欄位。")
                    if not df_item.empty and len(df_item.columns) > 0:
                        ...
                        data_frames[name] = pd.DataFrame(columns=[name]).set_index(pd.to_datetime([]))
    ...
    def _preprocess_and_align(self, raw_data: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        # 過濾掉空的 DataFrame
        valid_data_frames = {name: df for name, df in raw_data.items() if not df.empty and isinstance(df.index, pd.DatetimeIndex)}
        ...
        for name, df_item in valid_data_frames.items():
            if not isinstance(df_item.index, pd.DatetimeIndex): # 再次檢查
                 print(f"警告：在合併前發現 {name} 的索引不是 DatetimeIndex。跳過此數據。")
                 continue
        ...
        print(f"--- [階段 2] 數據預處理與對齊完成。最終用於標準化的數據維度: {aligned_df.shape} ---")
    ...
    def _normalize_to_zscore(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in df.columns:
            ...
            if series_to_normalize.isnull().all():
                print(f"警告：欄位 {col} 在轉換為數值型後全為 NaN，無法計算 Z-score。跳過此欄位。")
                zscore_df[f"{col}_zscore"] = pd.NA # 或 pd.Series([pd.NA]*len(df), index=df.index)
                continue

            rolling_mean = series_to_normalize.rolling(window=self.rolling_window, min_periods=int(self.rolling_window/2)).mean()
            rolling_std = series_to_normalize.rolling(window=self.rolling_window, min_periods=int(self.rolling_window/2)).std()
            ...
            print(f"資訊：已計算 {col}_zscore。有效值數量: {zscore_values.notna().sum()}/{len(zscore_values)}")
    ...
    def _aggregate_index(self, zscores: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'NYFed_Positions_zscore' in adjusted_zscores.columns:
            ...
            print("資訊：已反轉 'NYFed_Positions_zscore' 方向 (假設持有量越高壓力越小)。")
        ...
        # 確保只對實際存在的欄位進行平均
        valid_zscore_cols = [col for col in adjusted_zscores.columns if adjusted_zscores[col].notna().any()]
...
def plot_stress_index(df: pd.DataFrame, title: str = '每日綜合金融壓力指數'):
    ...
    fig.add_hline(y=0, line_width=1, line_dash="dash", line_color="gray",
                  annotation_text="基準線", annotation_position="bottom right")

    # 可以考慮添加一些統計線，例如 +1/-1 標準差
    mean_val = df['Stress_Index'].mean()
    std_val = df['Stress_Index'].std()
    fig.add_hline(y=mean_val + std_val, line_width=1, line_dash="dot", line_color="rgba(0,100,80,0.5)",
...
if __name__ == '__main__':
    ...
    try:
        ...
        # 4. 繪製圖表
        if not final_stress_index.empty:
            plot_stress_index(final_stress_index, title="每日綜合金融壓力指數 (滾動窗口 2年)")
        else:
            print("\n錯誤：最終壓力指數為空，無法繪製圖表。")
