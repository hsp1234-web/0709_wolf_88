...
class TimeAggregatorStep(BaseETLStep):
    ...
    def __init__(self, aggregation_level: str = "1Min",
                 db_writer_step: BaseETLStep | None = None):
        """
        ...
        self.logger.info(f"TimeAggregatorStep initialized for aggregation level: {aggregation_level} (Pandas rule: {self.aggregation_level_pd})")
    ...
    def execute(self, data: pd.DataFrame | None = None) -> pd.DataFrame | None:
        ...
        if not pd.api.types.is_datetime64_any_dtype(data['timestamp']):
            ...
            try:
                ...
                self.logger.error(f"Failed to convert 'timestamp' column to datetime: {e}", exc_info=True)
        ...
        for instrument, group_df in ticks_df.groupby('instrument'):
            self.logger.debug(f"Aggregating for instrument: {instrument}, {len(group_df)} ticks, rule: {self.aggregation_level_pd}")
            ...
            try:
                ...
                self.logger.error(f"Error during resampling for instrument {instrument} with rule '{self.aggregation_level_pd}': {e}", exc_info=True)
                continue

            if resampled_group.empty:
                self.logger.debug(f"No data after resampling for instrument {instrument} at {self.aggregation_level_pd} interval.")
                continue

            resampled_group.columns = ['_'.join(col).strip() for col in resampled_group.columns.values]

            resampled_group.rename(columns={
                'price_first': 'open',
                'price_max': 'high',
                'price_min': 'low',
                'price_last': 'close',
        ...
        output_columns = ['timestamp', 'instrument', 'open', 'high', 'low', 'close', 'volume']
        ...
        self.logger.info(f"TimeAggregatorStep successfully aggregated {len(data)} ticks into {len(final_ohlcv_df)} OHLCV records.")
...
if __name__ == '__main__':
    ...
    sample_ticks_data = [
        {"timestamp": datetime.datetime(2023, 1, 1, 9, 0, 5), "price": 100.0, "volume": 10, "instrument": "INSTR_A"},
        {"timestamp": datetime.datetime(2023, 1, 1, 9, 0, 15), "price": 101.0, "volume": 5, "instrument": "INSTR_A"},
        {"timestamp": datetime.datetime(2023, 1, 1, 9, 0, 30), "price": 99.0, "volume": 8, "instrument": "INSTR_A"},
        {"timestamp": datetime.datetime(2023, 1, 1, 9, 0, 50), "price": 100.5, "volume": 12, "instrument": "INSTR_A"},
        {"timestamp": datetime.datetime(2023, 1, 1, 9, 1, 10), "price": 102.0, "volume": 7, "instrument": "INSTR_A"},
        {"timestamp": datetime.datetime(2023, 1, 1, 9, 0, 10), "price": 2000.0, "volume": 20, "instrument": "INSTR_B"},
        {"timestamp": datetime.datetime(2023, 1, 1, 9, 0, 40), "price": 1995.0, "volume": 15, "instrument": "INSTR_B"},
    ]
