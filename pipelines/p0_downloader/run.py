...
def execute_download(session, task_info, output_dir):
    ...
    for attempt in range(3): # 重試3次
        try:
            ...
            response = session.post(task_info['url'], data=task_info.get('payload', {}), headers=headers, timeout=120) if task_info.get('payload') else session.get(task_info['url'], headers=headers, timeout=120)
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
        with requests.Session() as session:
            future_to_task = {executor.submit(execute_download, session, task, args.output_dir): task for task in tasks}
