...
except NameError: # 通常在交互式環境或特殊執行上下文中發生
    project_root = Path(os.getcwd()) # 退回到當前工作目錄
    if str(project_root) not in sys.path:
        sys.path.insert(0, str(project_root))
    print(f"警告：__file__ 未定義於 _test_harness_taifex_transformer.py，專案路徑校正可能不準確。", file=sys.stderr)
# --- 標準化「路徑自我校正」樣板碼 END ---
