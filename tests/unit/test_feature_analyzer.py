...
try:
    ...
    if str(project_root) not in sys.path:
        sys.path.insert(0, str(project_root))
    print(f"警告：(test_feature_analyzer.py) __file__ 未定義，專案路徑校正可能不準確。已將 {project_root} 加入 sys.path。", file=sys.stderr)
except Exception as e:
    print(f"專案路徑校正時發生錯誤 (tests/unit/test_feature_analyzer.py): {e}", file=sys.stderr)
# --- 標準化「路徑自我校正」樣板碼 END ---
