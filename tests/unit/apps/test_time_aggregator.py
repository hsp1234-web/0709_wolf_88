...
def assert_df_equals(df1, df2, check_dtype=True, rtol=1e-5, atol=1e-8):
    ...
    pd.testing.assert_frame_equal(
        df1, df2, check_dtype=check_dtype, rtol=rtol, atol=atol
    )
...
class TestTimeAggregator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sample_ticks_data = [
            ...
            {
                "timestamp": self.sample_start_time + datetime.timedelta(seconds=10),
                "price": 100.0,
                "volume": 10,
                "instrument": "INST_A",
            },  # O=100
            {
                "timestamp": self.sample_start_time + datetime.timedelta(seconds=20),
                "price": 102.0,
                "volume": 5,
                "instrument": "INST_A",
            },  # H=102
            {
                "timestamp": self.sample_start_time + datetime.timedelta(seconds=30),
                "price": 99.0,
                "volume": 8,
                "instrument": "INST_A",
            },  # L=99
            {
                "timestamp": self.sample_start_time + datetime.timedelta(seconds=50),
                "price": 101.0,
                "volume": 12,
                "instrument": "INST_A",
            },  # C=101, V=10+5+8+12=35 (Minute 1)
            {
                "timestamp": self.sample_start_time
            ...
            {
                "timestamp": self.sample_start_time
                + datetime.timedelta(minutes=1, seconds=45),
                "price": 101.0,
                "volume": 9,
                "instrument": "INST_A",
            },  # L=101, C=101, V=7+4+9=20 (Minute 2)
            # Instrument B: 1 分鐘的數據 (跨越到第二分鐘，但只有一筆)
            {
                "timestamp": self.sample_start_time + datetime.timedelta(seconds=15),
                "price": 2000.0,
                "volume": 20,
                "instrument": "INST_B",
            },  # O=2000, H=2000
            {
                "timestamp": self.sample_start_time + datetime.timedelta(seconds=40),
                "price": 1995.0,
                "volume": 15,
                "instrument": "INST_B",
            },  # L=1995, C=1995, V=20+15=35 (Minute 1)
            {
                "timestamp": self.sample_start_time
                ...
                "volume": 25,
                "instrument": "INST_B",
            },  # O,H,L,C=1998, V=25 (Minute 2)
            # Instrument C: 只有一筆交易，測試單點情況
            {
                "timestamp": self.sample_start_time + datetime.timedelta(seconds=35),
                "price": 50.0,
                "volume": 1,
                "instrument": "INST_C",
            },  # O,H,L,C=50, V=1 (Minute 1)
        ]
        self.sample_ticks_df = pd.DataFrame(self.sample_ticks_data)
        self.sample_ticks_df["timestamp"] = pd.to_datetime(
            self.sample_ticks_df["timestamp"]
        )
    ...
    def test_read_bronze_ticks(self, mock_duckdb_connect):
        ...
        result_df = self.aggregator.read_bronze_ticks(
            start_query_time, end_query_time, "bronze_test_table"
        )
    ...
    def test_read_bronze_ticks_handles_db_error(self, mock_duckdb_connect):
        ...
        mock_conn = MagicMock()
        mock_duckdb_connect.return_value = mock_conn
        mock_conn.execute.return_value.fetchdf.side_effect = Exception(
            "Simulated DB error"
        )
        ...
        self.assertEqual(
            list(result_df.columns), ["timestamp", "price", "volume", "instrument"]
        )
    ...
    def test_aggregate_to_1m_ohlcv_correctness(self):
        ...
        """
        result_ohlcv_df = self.aggregator.aggregate_to_1m_ohlcv(
            self.sample_ticks_df.copy()
        )  # 使用副本
        ...
        expected_data = [
            {
                "timestamp": pd.Timestamp(self.sample_start_time),
                "instrument": "INST_A",
                "open": 100.0,
                "high": 102.0,
                "low": 99.0,
            ...
            },
            {
                "timestamp": pd.Timestamp(
                    self.sample_start_time + datetime.timedelta(minutes=1)
                ),
                "instrument": "INST_A",
                "open": 101.5,
                "high": 103.0,
        ...
        # 排序以確保比較的一致性
        result_ohlcv_df = result_ohlcv_df.sort_values(
            by=["instrument", "timestamp"]
        ).reset_index(drop=True)
        expected_ohlcv_df = expected_ohlcv_df.sort_values(
            by=["instrument", "timestamp"]
        ).reset_index(drop=True)
    ...
    def test_aggregate_to_1m_ohlcv_no_trades_in_minute(self):
        ...
        ticks_data = [
            ...
            {
                "timestamp": self.sample_start_time
                + datetime.timedelta(minutes=2, seconds=10),
                "price": 102.0,
                "volume": 5,
                "instrument": "INST_A",
            },
        ...
        self.assertNotIn(
            pd.Timestamp(self.sample_start_time + datetime.timedelta(minutes=1)),
            result_df["timestamp"].tolist(),
        )
    ...
    def test_write_silver_ohlcv(self, mock_duckdb_connect):
        ...
        self.aggregator.write_silver_ohlcv(
            ohlcv_df, silver_table_name=silver_table_name
        )
        ...
        for c in mock_conn.execute.call_args_list:
            if "CREATE TABLE IF NOT EXISTS silver_ohlcv_test" in c.args[0]:
                create_table_call = c
            if (
                f"INSERT INTO {silver_table_name}" in c.args[0]
            ):  # 或是 duckdb 的 append 語法
                insert_call = c
        ...
        self.assertIn(
            f"INSERT INTO {silver_table_name} SELECT * FROM ohlcv_df_temp_view",
            insert_call.args[0],
        )
...
class TestTimeAggregatorRunScript(unittest.TestCase):
    ...
    def test_run_main_flow(self, mock_path_exists, mock_os_remove, MockTimeAggregator):
        ...
        mock_aggregator_instance = (
            MockTimeAggregator.return_value.__enter__.return_value
        )

        # 模擬 aggregate_to_1m_ohlcv 返回一個非空的 DataFrame，以觸發 write_silver_ohlcv
        mock_ohlcv_output_df = pd.DataFrame({"col1": [1]})  # 內容不重要，只需非空
        mock_aggregator_instance.aggregate_to_1m_ohlcv.return_value = (
            mock_ohlcv_output_df
        )
    ...
    def test_run_main_flow_empty_aggregation_skips_write(
        self, mock_path_exists, mock_os_remove, MockTimeAggregator
    ):
        """
    ...
    @patch(
        "apps.time_aggregator.run.TimeAggregator"
    )  # 仍然需要模擬 TimeAggregator 本身
    @patch("apps.time_aggregator.run.os.remove")
    @patch("apps.time_aggregator.run.os.path.exists")
    def test_run_main_cleans_db_files_when_run_as_main(
        self, mock_path_exists, mock_os_remove, MockTimeAggregator
    ):
        """
        ...
        mock_aggregator_instance.aggregate_to_1m_ohlcv.return_value = pd.DataFrame(
            {"a": [1]}
        )
