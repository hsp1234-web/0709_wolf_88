...
class TestLoader(unittest.TestCase):
    ...
    def setUp(self):
        ...
        pd.DataFrame({"colA": [1, 2], "colB": ["val1", "val2"]}).to_parquet(
            self.dummy_parquet_path
        )

        self.test_argv = [
            "--parquet-file",
            str(self.dummy_parquet_path),
            "--db-path",
            str(self.dummy_db_path),
            "--table-name",
            self.table_name,
            "--primary-key",
            "colA",  # Assuming colA is a primary key for some tests
            "--loglevel",
            "DEBUG",
        ]
    ...
    @patch(
        "apps.etl_pipeline.loader._get_parquet_schema"
    )  # 新增對 _get_parquet_schema 的 mock
    def test_run_loading_successful_creation(
        self, mock_get_pq_schema, mock_duckdb_connect
    ):
        """
        ...
        mock_get_pq_schema.return_value = [
        ...
        mock_duckdb_connect.assert_called_with(
            database=str(self.dummy_db_path), read_only=False
        )
        ...
        for call_args in mock_conn.execute.call_args_list:
            ...
            current_sql_call_for_test = " ".join(
                str(call_args[0][0]).split()
            )  # keep original case for quoted table name part
            ...
            if (
                expected_part1_loader_case
                in current_sql_call_for_test  # Check with original case for quoted table name
                and expected_part2
                in executed_sql_normalized  # Check AS SELECT in uppercase normalized
                and expected_part3 in executed_sql_normalized
            ):  # Check FROM READ_PARQUET in uppercase normalized
                create_table_as_select_found = True
                break
        self.assertTrue(
            create_table_as_select_found,
            f'CREATE TABLE "{self.table_name}" AS SELECT ... FROM READ_PARQUET(...) 應被調用. \nCaptured calls:\n{[str(c[0][0]) for c in mock_conn.execute.call_args_list]}',
        )
    ...
    @patch(
        "apps.etl_pipeline.loader._compare_schemas", return_value=True
    )  # Assume schemas match
    def test_run_loading_successful_append_with_pk(
        ...
        mock_get_db_schema.return_value = [
            {"name": "colA", "type": "INTEGER"},
            {"name": "colB", "type": "VARCHAR"},
        ]
        mock_get_pq_schema.return_value = [
        ...
        for call in mock_conn.execute.call_args_list:
            ...
            normalized_target_table_check = " ".join(
                target_table_check.upper().split()
            )  # 用於與 normalized_sql 比較
            ...
            if (
                normalized_target_table_check in normalized_sql
                and not_exists_check in normalized_sql
            ):
                insert_call_found = True
                break

        # Define expected message parts outside the loop for the assert message
        expected_msg_part_table = (
            f'INSERT INTO "{self.table_name.upper()}"'  # As it's normalized for check
        )
        expected_msg_part_condition = "WHERE NOT EXISTS"
        self.assertTrue(
            insert_call_found,
            f"冪等 INSERT 查詢 (包含 '{expected_msg_part_table}' 和 '{expected_msg_part_condition}') 應被調用. \nCaptured calls:\n{[str(c[0][0]) for c in mock_conn.execute.call_args_list]}",
        )
    ...
    def test_run_loading_missing_arguments(self):
        ...
        with self.assertRaises(SystemExit) as cm:
            loader.run_loading(
                ["--db-path", str(self.dummy_db_path), "--table-name", self.table_name]
            )
    ...
    @patch(
        "apps.etl_pipeline.loader._load_parquet_to_db_internal",
        side_effect=SchemaMismatchError("Mocked schema mismatch"),
    )
    ...
    def test_run_loading_parquet_file_not_found(self):
        ...
        with patch(
            "apps.etl_pipeline.loader.Path.exists",
            side_effect=lambda p: False if "non_existent.parquet" in str(p) else True,
        ):
