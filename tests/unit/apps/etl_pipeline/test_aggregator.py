...
class TestAggregator(unittest.TestCase):
    ...
    @patch(
        "apps.etl_pipeline.aggregator.pd.DataFrame.to_sql", new_callable=MagicMock
    )  # 如果有用 to_sql
    @patch(
        "apps.etl_pipeline.aggregator.pd.read_sql_query", new_callable=MagicMock
    )  # 如果有用 read_sql_query
    def test_run_aggregation_successful_execution(
        ...
        with patch(
            "apps.etl_pipeline.aggregator.pd.DataFrame.resample"
        ) as mock_resample:
            ...
            mock_duckdb_connect.assert_any_call(
                database="dummy_source.db", read_only=True
            )
            mock_duckdb_connect.assert_any_call(
                database="dummy_analytics.db", read_only=False
            )
            mock_conn.execute.assert_any_call(
                unittest.mock.ANY, [self.product_id, self.start_date, self.end_date]
            )  # 檢查查詢 tick
    ...
    def test_run_aggregation_missing_arguments(self):
        ...
        with self.assertRaises(SystemExit) as cm:
            aggregator.run_aggregation(
                [self.product_id, self.start_date]
            )  # 缺少 end_date
    ...
    def test_run_aggregation_source_db_not_exists_and_cannot_create_dummy(
        ...
        with patch(
            "apps.etl_pipeline.aggregator._create_dummy_source_db_if_needed",
            side_effect=RuntimeError("Cannot create dummy DB"),
        ):
            ...
            test_argv_for_dummy_fail = [
                ...
                self.end_date,
                "--source_db",
                str(aggregator.DEFAULT_SOURCE_TICKS_DB_PATH),  # 確保觸發 dummy 邏輯
                "--analytics_db",
                "dummy_analytics.db",
            ]
            result = aggregator.run_aggregation(test_argv_for_dummy_fail)
            self.assertFalse(
                result, "run_aggregation 應在無法處理來源資料庫時返回 False"
            )

    @patch(
        "apps.etl_pipeline.aggregator._ensure_db_directory_exists",
        side_effect=Exception("Dir creation failed"),
    )
