...
class TestGoldLayerBuilder(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for inst in self.instruments:
            for day_idx in range(self.num_days_for_features):
                day_base_time = self.silver_start_time + datetime.timedelta(
                    days=day_idx
                )
        ...
        self.sample_silver_df["timestamp"] = pd.to_datetime(
            self.sample_silver_df["timestamp"]
        )
    ...
    def test_read_silver_ohlcv_1m(self, mock_duckdb_connect):
        ...
        expected_df_data = [
            {
                "timestamp": self.silver_start_time,
                "instrument": "FXA_USD",
                "open": 100,
                "high": 101,
                "low": 99,
        ...
        self.assertIn(
            "SELECT timestamp, instrument, open, high, low, close, volume", args[0]
        )
    ...
    def test_aggregate_to_daily_ohlcv(self):
        ...
        daily_agg_test_data.extend(
            [
                ...
                {
                    "timestamp": test_dt_base + datetime.timedelta(minutes=30),
                    "instrument": "TEST01",
                    "open": 10.1,
                    "high": 10.5,
                    "low": 10.0,
                ...
                {
                    "timestamp": test_dt_base + datetime.timedelta(hours=1),
                    "instrument": "TEST01",
                    "open": 10.3,
                    "high": 10.4,
                    "low": 9.5,
                ...
                {
                    "timestamp": test_dt_base + datetime.timedelta(hours=2),
                    "instrument": "TEST01",
                    "open": 9.6,
                    "high": 9.9,
                    "low": 9.5,
        ...
        assert_df_equals(
            result_daily_df, expected_daily_df, sort_by_cols=["instrument"]
        )

    def test_calculate_features_correctness(self):
        # 使用 setUp 中準備的 self.sample_silver_df，先聚合成日線
        daily_ohlcv_df = self.builder.aggregate_to_daily_ohlcv(
            self.sample_silver_df.copy()
        )

        # 確保有足夠數據 (self.num_days_for_features = 25)
        self.assertTrue(
            len(daily_ohlcv_df[daily_ohlcv_df["instrument"] == "FXA_USD"]) >= 20
        )
        ...
        for inst in self.instruments:
            ...
            inst_features_df = result_features_df[
                result_features_df["instrument"] == inst
            ].copy()
            ...
            pd.testing.assert_series_equal(
                ...
                check_dtype=False,
                rtol=1e-5,
                check_names=False,  # RSI 比較敏感，可能需要調整 rtol
            )
    ...
    def test_write_gold_tables(self, mock_duckdb_connect):
        ...
        final_df["date"] = pd.to_datetime(
            final_df["date"]
        ).dt.date  # 確保是 date object

        self.builder.write_gold_tables(
            final_df, "gold_ohlcv_test", "gold_features_test"
        )
        ...
        self.assertEqual(
            list(registered_df_ohlcv.columns),
...
class TestGoldLayerRunScript(unittest.TestCase):
    ...
    def test_run_main_flow_full_execution(
        self, mock_path_exists, mock_os_remove, MockGoldLayerBuilder
    ):
        mock_path_exists.return_value = False  # 避免清理邏輯
        mock_builder_instance = MockGoldLayerBuilder.return_value.__enter__.return_value

        # 模擬 builder 方法的返回值
        mock_daily_ohlcv_df = pd.DataFrame(
            {
        ...
        mock_builder_instance.aggregate_to_daily_ohlcv.return_value = (
            mock_daily_ohlcv_df
        )
        ...
        mock_builder_instance.aggregate_to_daily_ohlcv.assert_called_once_with(
            ANY
        )  # 輸入是模擬生成的DF
        ...
        # 它們應該是同一個對象或內容相同
        assert_df_equals(
            call_args_calc_features[0],
            mock_daily_ohlcv_df,
            sort_by_cols=["instrument", "date"],
        )
    ...
    @patch("apps.gold_layer_builder.run.os.remove")
    @patch("apps.gold_layer_builder.run.os.path.exists")
    def test_run_main_skips_if_daily_aggregation_empty(
        self, mock_path_exists, mock_os_remove, MockGoldLayerBuilder
    ):
