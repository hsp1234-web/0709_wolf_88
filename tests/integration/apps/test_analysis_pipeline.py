...
try:
    ...
    project_root = current_script_path.parent.parent.parent.parent # tests/integration/apps/test_analysis_pipeline.py -> project_root
...
class TestAnalysisPipelineRunScript(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.assertTrue(self.pipeline_script_path.exists(), f"Pipeline script not found at {self.pipeline_script_path}")
        ...
        # 清理舊的測試資料庫檔案 (如果存在)
        for db_p in [self.test_db_path, self.test_analytics_mart_db_path, self.test_legacy_quadrant_db_path]:
            if db_p.exists():
                db_p.unlink()
            # 確保目錄存在
    ...
    def _create_initial_tables(self):
        ...
        with duckdb.connect(str(self.test_db_path)) as con:
            con.execute("CREATE TABLE IF NOT EXISTS market_ohlcv_data (datetime TIMESTAMPTZ, ticker VARCHAR, interval VARCHAR, open DOUBLE, high DOUBLE, low DOUBLE, close DOUBLE, volume BIGINT, PRIMARY KEY (ticker, datetime, interval));")
            con.execute("CREATE TABLE IF NOT EXISTS FactorStore_Daily (ticker TEXT, date DATE, factor_name TEXT, factor_value REAL, PRIMARY KEY (ticker, date, factor_name));")
            # StrategicAnalyzer 的 _get_latest_market_price 依賴 MarketPrices_Daily
            con.execute("CREATE TABLE IF NOT EXISTS MarketPrices_Daily (datetime TIMESTAMPTZ, ticker VARCHAR, interval VARCHAR, open REAL, high REAL, low REAL, close REAL, volume BIGINT, PRIMARY KEY (ticker, datetime, interval));")
            # StrategicAnalyzer 的 _save_results 寫入 StrategicDashboard_Daily
            con.execute("CREATE TABLE IF NOT EXISTS StrategicDashboard_Daily (date DATE, indicator_name TEXT, signal TEXT, value REAL, commentary TEXT, PRIMARY KEY (date, indicator_name));")


        # 為 chimera (使用 test_analytics_mart_db_path)
        with duckdb.connect(str(self.test_analytics_mart_db_path)) as con:
            con.execute("CREATE TABLE IF NOT EXISTS ohlcv_1d (timestamp TIMESTAMP, product_id VARCHAR, open DOUBLE, high DOUBLE, low DOUBLE, close DOUBLE, volume BIGINT, PRIMARY KEY (timestamp, product_id));")
            ...
            con.execute("CREATE TABLE IF NOT EXISTS daily_ohlc (trading_date DATE, product_id VARCHAR, expiry_month VARCHAR, strike_price DOUBLE, option_type VARCHAR, open DOUBLE, high DOUBLE, low DOUBLE, close DOUBLE, settlement_price DOUBLE, volume UBIGINT, open_interest UBIGINT, trading_session VARCHAR, source_file VARCHAR, member_file VARCHAR, transformed_at TIMESTAMPTZ, PRIMARY KEY (trading_date, product_id, option_type, strike_price, expiry_month));") # 假設更完整的主鍵
        ...
        with duckdb.connect(str(self.test_legacy_quadrant_db_path)) as con:
            ...
            from apps.analysis_pipeline.run import TIME_PERIODS_FOR_LEGACY_QUADRANT # 獲取時間週期
            for period in TIME_PERIODS_FOR_LEGACY_QUADRANT.keys():
                con.execute(f"CREATE TABLE IF NOT EXISTS ohlcv_{period} (timestamp TIMESTAMP, product_id VARCHAR, close DOUBLE, volume BIGINT, PRIMARY KEY (timestamp, product_id));")
...
class TestAnalysisPipelineMainFunction(unittest.TestCase):
    ...
    def test_main_calls_daily_market_analyzer(self, MockDBManager, MockDailyMarketAnalysisEngine):
        ...
        test_args = [ # sys.argv 的模擬列表
            ...
            "--analyzer", "daily_market",
            "--tickers", "AAPL,MSFT",
            "--start_date", "2023-01-01",
            "--db_path", "dummy_market.db", # 使用虛擬路徑
            "--dma_table_name", "market_data"
        ]
    ...
    def test_main_calls_chimera_composite(self, MockChimeraAnalyzer):
        ...
        test_args = [
            "run.py",
            "--analyzer", "chimera_composite",
            "--analytics_mart_db", "dummy_analytics.db", # 使用虛擬路徑
    ...
    def test_main_calls_chimera_pc_ratio(self, MockChimeraAnalyzer):
        ...
        test_args = [
            "run.py",
            "--analyzer", "chimera_pc_ratio",
            "--analytics_mart_db", "dummy_analytics.db", # 使用虛擬路徑
            "--start_date", "2023-02-01", "--end_date", "2023-02-05",
            "--pc_ratio_products", "TXO", "TEO"
        ]
        with patch.object(sys, 'argv', test_args):
            from apps.analysis_pipeline import run as analysis_run
            analysis_run.main()
    ...
    def test_main_calls_legacy_quadrant(self, MockLegacyQuadrantAnalyzer):
        ...
        test_args = [
            "run.py",
            "--analyzer", "legacy_quadrant",
            "--legacy_quadrant_db", "dummy_legacy.db" # 使用虛擬路徑
        ]
        with patch.object(sys, 'argv', test_args):
            from apps.analysis_pipeline import run as analysis_run
    ...
    def test_main_calls_institutional_analyzer(self, MockInstitutionalAnalyzer): # 只接收一個 mock 參數
        ...
        test_args = [
            "run.py",
            "--analyzer", "institutional",
            "--analytics_mart_db", "dummy_analytics.db",
            "--stock_ids", "0050",
            "--start_date", "2023-03-01", "--end_date", "2023-03-05",
            "--finmind_api_token", "fake_token"
        ]
        with patch.object(sys, 'argv', test_args):
            from apps.analysis_pipeline import run as analysis_run
            analysis_run.main()
