...
def generate_status_table(metrics: dict) -> Table:
    ...
    if not checkpoints:
        ...
        for consumer_id, last_id in checkpoints.items():
            ...
            table.add_row(f"消費者 [{consumer_id}] 延遲", f"[red]{lag}[/red]" if lag > 10 else f"[green]{lag}[/green]")
            table.add_row("---", "---")

    if metrics.get("status") == "關閉中...":
        table.add_row("[bold yellow]系統狀態[/bold yellow]", "[bold yellow]關閉中...[/bold yellow]")
