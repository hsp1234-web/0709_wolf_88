...
from prometheus.core.config import get_fred_api_key  # **關鍵改造**：導入金鑰獲取函數
...
class FredClient(BaseAPIClient):
    ...
    def __init__(
        ...
        if api_key:
            ...
            try:
                print(
                    "資訊：FredClient 初始化時未直接傳入 API 金鑰，嘗試從設定檔獲取..."
                )
        ...
        if not final_api_key:  # 雙重檢查，確保 final_api_key 有值
            error_msg = (
                "FredClient 初始化失敗：API 金鑰既未直接提供，也無法從設定檔中獲取。"
            )
        ...
        print(
            f"資訊：FredClient ({self.__class__.__name__}) 初始化成功。FredAPILib 將自行管理其網路請求。已啟用應急快取。"
        )
    ...
    def fetch_data(self, symbol: str, **kwargs: Any) -> pd.DataFrame:
        ...
        if not force_refresh and cache_key in self._emergency_cache:
            print(
                f"資訊：{self.__class__.__name__} 使用應急快取獲取指標 {symbol} 及參數 {cache_key_params}..."
            )
        ...
        with self._get_request_context(
            force_refresh=force_refresh
        ):  # force_refresh 仍可用於清空應急快取
            if force_refresh and cache_key in self._emergency_cache:
                print(
                    f"資訊：{self.__class__.__name__} 應急快取因 force_refresh 而清除 {symbol} / {cache_key_params}。"
                )
            ...
            try:
                ...
                print(
                    f"資訊：{self.__class__.__name__} 正在透過 FredAPILib 請求 {symbol}..."
                )
                series_data = self._fred_official_client.get_series(
                    series_id=symbol, **fred_params
                )
            except Exception as e:  # 捕獲 fredapi 可能拋出的任何錯誤
                print(
                    f"錯誤：{self.__class__.__name__} 使用 fredapi 獲取 {symbol} 時發生錯誤。類型: {type(e)}, 訊息: {str(e)}"
                )
        ...
        if not isinstance(series_data, pd.Series):
            print(
                f"警告：{self.__class__.__name__} 從 FRED 獲取的指標 {symbol} 數據類型不是 pd.Series，而是 {type(series_data)}。"
            )
            return pd.DataFrame(columns=["Date", symbol]).set_index("Date")

        if series_data.empty:
            print(
                f"警告：{self.__class__.__name__} 從 FRED 獲取的指標 {symbol} 數據為空。"
            )
            return pd.DataFrame(columns=["Date", symbol]).set_index("Date")
        ...
        if df.empty or (symbol in df and df[symbol].isnull().all()):
            print(
                f"警告：{self.__class__.__name__} 獲取的指標 {symbol} 數據轉換後無效或全為空值。"
            )
            return pd.DataFrame(columns=["Date", symbol]).set_index("Date")
        ...
        print(
            f"資訊：{self.__class__.__name__} 成功獲取並已存入應急快取 {len(df)} 筆 {symbol} / {cache_key_params} 數據。"
        )
        return df
