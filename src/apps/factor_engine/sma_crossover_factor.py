...
def calculate_sma_crossover(
    ...
    """
    print(
        f"--- 開始計算 {ticker.upper()} 的 "
        f"{short_window}H/{long_window}H SMA 交叉因子 ---"
    )
    ...
    result_df["short_sma"] = (
        result_df[f"{ticker}_close"].rolling(window=short_window).mean()
    )
    result_df["long_sma"] = (
        result_df[f"{ticker}_close"].rolling(window=long_window).mean()
    )
