...
def main():
    ...
    try:
        ...
        logger.error(
            f"無法從數據庫讀取數據，請先執行數據管線。錯誤: {e}", exc_info=True
        )
        return
    ...
    fig.add_trace(
        go.Bar(
            x=df["timestamp"], y=df["spy_gex_total"], name="GEX", marker_color=colors
        ),
        row=2,
        col=1,
    )
