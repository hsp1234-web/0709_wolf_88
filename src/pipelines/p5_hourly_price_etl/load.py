...
def run_load(transformed_df: pd.DataFrame, mode: str):
    ...
    try:
        with DBManager() as db_manager:
            ...
            if mode == "update":
                try:
                    ...
                    logger.info(
                        f"更新模式：將刪除 '{table_name}' 中 timestamp >= '{min_ts_naive}' 的現有數據。"
                    )
                    delete_query = (
                        f"DELETE FROM {table_name} WHERE timestamp >= '{min_ts_naive}'"
                    )
                    ...
                    existing_columns = [
                        desc[0]
                        for desc in db_manager.connection.execute(
                            f"DESCRIBE {table_name}"
                        ).fetchall()
                    ]

                    # 確保新數據的欄位與現有表格一致
                    df_to_load = df_to_load.reindex(columns=existing_columns).fillna(
                        value=pd.NA
                    )
                    df_to_load["timestamp"] = pd.to_datetime(
                        df_to_load["timestamp"], unit="ns", utc=True
                    )
        ...
        logger.info(
            f"--- [Loader] 完成，成功將 {len(df_to_load)} 筆數據加載到 '{table_name}' ---"
        )
...
if __name__ == "__main__":
    ...
    mock_df_1 = pd.DataFrame(
        {"spy_close": [400, 401, 402], "qqq_close": [300, 301, 302]},
    ...
    try:
        ...
        with DBManager() as db:
            ...
            # 驗證 12:00 的數據是新的
            assert (
                result[result["timestamp"] == pd.to_datetime("2023-01-01 12:00:00")][
                    "spy_close"
                ].iloc[0]
                == 402.5
            )
