...
def run_transformation(raw_data: Dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    for ticker, df in raw_data.items():
        ...
        safe_ticker = (
            ticker.lower().replace("-", "_").replace("=f", "_f").replace("^", "")
        )
...
def calculate_technical_indicators(price_df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    logger.info(
        f"--- [Indicator Calculator] 啟動，準備為 {price_df.shape[0]} 行數據計算指標 ---"
    )
    ...
    assets = sorted(
        list(set([col.split("_")[0] for col in price_df.columns if "_" in col]))
    )
    ...
    if all_indicators:
        ...
        logger.info(
            f"--- [Indicator Calculator] 完成，新增 {len(indicators_merged.columns)} 個指標欄位 ---"
        )
    else:
        final_df = price_df
...
def calculate_options_derived_metrics(price_df: pd.DataFrame) -> pd.DataFrame:
        ...
        with DBManager() as db:
            ...
            if "daily_macro_market_data" in tables_df["name"].values:
                ...
                if "DGS3MO" in daily_data.columns and not daily_data["DGS3MO"].empty:
                    ...
                    logger.warning(
                        "日線數據表中缺少 'DGS3MO' 欄位或該欄位為空，將使用預設無風險利率。"
                    )
            else:
                logger.warning(
                    "日線數據表 'daily_macro_market_data' 不存在，將使用預設無風險利率。"
                )
    except Exception as e:
    ...
    if "gamma" in calls.columns and "gamma" in puts.columns:
        ...
        total_gex = (calls["gamma"] * calls["openInterest"] * 100).sum() - (
            puts["gamma"] * puts["openInterest"] * 100
        ).sum()
    else:
    ...
    for strike in strikes:
        call_loss = (
            ((calls["strike"] - strike) * calls["openInterest"]).clip(lower=0).sum()
        )
        put_loss = (
            ((strike - puts["strike"]) * puts["openInterest"]).clip(lower=0).sum()
        )
...
if __name__ == "__main__":
    ...
    mock_raw_data = {
        ...
        "GC=F": pd.DataFrame(
            ...
            index=pd.to_datetime(["2023-01-02 09:30:00", "2023-01-02 11:30:00"]),
        ),  # 注意時間戳不同
        "BRK-B": pd.DataFrame(
            {  # 測試特殊字符
                "Open": [410, 411],
