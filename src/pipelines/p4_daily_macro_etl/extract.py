...
def run_extraction(
    ...
    """
    logger.info(
        f"數據提取流程開始。強制下載模式: {'啟用' if force_download else '停用'}"
    )
    ...
    try:
        ...
        with (
            temporary_disabled_cache(session)
            if force_download
            else open("/dev/null", "w") as _
        ):
            for series_id, description in fred_series_to_fetch.items():
                try:
                    ...
                    if not df.empty:
                        ...
                        logger.warning(
                            f"從 FRED 提取 {series_id} 時返回了空的 DataFrame。"
                        )
        ...
        for symbol, description in yfinance_symbols_to_fetch.items():
            try:
                ...
                if not df.empty:
                    ...
                    logger.warning(
                        f"在指定期間內從 yfinance 提取 {symbol} 時返回了空的 DataFrame。"
                    )
            except Exception as e:
