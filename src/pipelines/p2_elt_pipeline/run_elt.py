...
from pipelines.p1_explorer.run import (  # noqa: E402
...
def run_loader(input_dir, raw_db_path, schema_db_path):
    ...
    if not os.path.exists(schema_db_path):
        print(
            f"[WARNING] Loader: Schema registry DB {schema_db_path} not found. No files will be loaded."
        )
    else:
        try:
            schema_conn = sqlite3.connect(schema_db_path)
            # Check if schema_registry table exists before querying
            table_exists_cursor = schema_conn.execute(
                "SELECT name FROM sqlite_master WHERE type='table' AND name='schema_registry';"
            )
            if table_exists_cursor.fetchone():
                known_fingerprints = {
                    row[0]
                    for row in schema_conn.execute(
                        "SELECT format_fingerprint FROM schema_registry"
                    ).fetchall()
                }
            else:
                print(
                    f"[WARNING] Loader: 'schema_registry' table not found in {schema_db_path}. No files will be loaded based on schema."
                )
            schema_conn.close()
        except sqlite3.Error as e:
            print(
                f"[ERROR] Loader: Error reading schema registry {schema_db_path}: {e}. No files will be loaded."
            )

    if not known_fingerprints:
        print(
            "[INFO] Loader: No known fingerprints loaded from schema registry. Only files matching these will be processed."
        )
    ...
    if not os.path.exists(input_dir):
        print(
            f"[WARNING] Loader input directory {input_dir} does not exist. Skipping loading."
        )
        raw_conn.close()
        return
    ...
    for filename in os.listdir(input_dir):
        ...
        if (
            ...
            print(
                f"[INFO] Loader: File {filename} already in raw_import_log. Skipping."
            )
            continue
        ...
        try:
            ...
            if zipfile.is_zipfile(file_path):
                try:
                    with zipfile.ZipFile(file_path, "r") as zf:
                        if zf.namelist():
                            member = zf.namelist()[
                                0
                            ]  # Process only the first member for prospecting
                            # Check if member is csv or txt before reading
                            if member.lower().endswith((".csv", ".txt")):
                                file_bytes_content = zf.read(member)
                            else:
                                print(
                                    f"[INFO] Loader: Member {member} in zip {filename} is not CSV/TXT. Skipping zip for schema check."
                                )
                                continue  # Skip this zip if first member isn't CSV/TXT
                        else:
                            print(
                                f"[WARNING] Loader: Zip file {filename} is empty. Skipping."
                            )
                            continue
                except zipfile.BadZipFile:
                    print(
                        f"[WARN] Loader: File {filename} is a bad zip file. Skipping."
                    )
                    continue
                ...
                print(
                    f"[INFO] Loader: File {filename} is not a processable type (zip, csv, txt) for schema check. Skipping."
                )
                continue
            ...
            if file_bytes_content is None:
                ...
                print(
                    f"[INFO] Loader: No content to process for {filename} after type checks. Skipping."
                )
                continue
            ...
            if result["status"] == "success":
                ...
                if fingerprint in known_fingerprints:
                    raw_conn.execute(
                        ...
                        (file_path, file_bytes_content, fingerprint),
                    )
                    files_loaded += 1
                    print(
                        f"[INFO] Loader: Loaded {filename} (fingerprint: {fingerprint[:8]}...) as it's a known schema."
                    )
                else:
                    print(
                        f"[INFO] Loader: Skipped {filename} (fingerprint: {fingerprint[:8]}...) as its schema is not in the registry."
                    )
            else:
                print(
                    f"[INFO] Loader: Skipped file {filename} due to content prospecting failure: {result.get('error', 'Unknown error')}"
                )
...
def run_transformer(raw_db_path, schema_db_path, analytics_db_path):
    ...
    try:
        schema_exists = schema_conn_transformer.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='schema_registry';"
        ).fetchone()
        if not schema_exists:
            print(
                f"[WARNING] Transformer: schema_registry table not found in {schema_db_path}. Transformer cannot operate."
            )
        ...
        schema_map = {
            row[0]: (row[1], row[2])
            for row in schema_conn_transformer.execute(
                "SELECT format_fingerprint, header, encoding FROM schema_registry"
            ).fetchall()
        }
    except sqlite3.Error as e:
    ...
    if not schema_map:
        print(
            "[WARNING] Transformer: 格式註冊表為空或讀取失敗，Transformer 無法執行有效轉換。"
        )
    ...
    daily_futures_header_str = (
        "交易日期,契約代碼,到期月份(週別),開盤價,最高價,最低價,收盤價,成交量"
    )
    target_daily_futures_fingerprint = get_target_fingerprint(
        schema_map, daily_futures_header_str
    )

    if not target_daily_futures_fingerprint:
        print(
            f"[WARNING] Transformer: Did not find fingerprint for daily_futures_header '{daily_futures_header_str}' in schema_map. Cannot process daily_futures."
        )
        # Proceed to create empty table but don't try to load data for it.
    else:
        print(
            f"[INFO] Transformer: Target fingerprint for daily_futures is {target_daily_futures_fingerprint[:8]}..."
        )

    # Ensure raw_db_path directory exists
    os.makedirs(os.path.dirname(raw_db_path), exist_ok=True)
    raw_conn = duckdb.connect(
        raw_db_path, read_only=False
    )  # Changed to False to allow table creation if not exists
    ...
    raw_table_exists = raw_conn.execute(
        "SELECT table_name FROM information_schema.tables WHERE table_name = 'raw_import_log'"
    ).fetchone()
    if not raw_table_exists:
        print(
            f"[WARNING] Transformer: raw_import_log table not found in {raw_db_path}. Creating an empty one."
        )
        raw_conn.execute("""
        CREATE TABLE IF NOT EXISTS raw_import_log (
            file_path VARCHAR PRIMARY KEY,
        ...
        print(
            "Transformer 完成，成功轉換 0 筆記錄 (raw_import_log was missing or empty)."
        )
        return
    ...
    records = raw_conn.execute(
        "SELECT content_blob, format_fingerprint FROM raw_import_log"
    ).fetchall()
    transformed_count = 0
    for blob, fingerprint in records:
        ...
        if fingerprint != target_daily_futures_fingerprint:
            if (
                fingerprint in schema_map
            ):  # It's a known schema, just not the one we're processing now
                print(
                    f"[INFO] Transformer: Skipping record with fingerprint {fingerprint[:8]}... as it's not the target daily_futures fingerprint ({target_daily_futures_fingerprint[:8]}...)."
                )
            else:  # Should not happen if loader only loads known FPs, but as a safeguard:
                print(
                    f"[INFO] Transformer: Fingerprint {fingerprint[:8]}... not in schema_map (and not target). Skipping."
                )
            continue
        ...
        if target_daily_futures_fingerprint not in schema_map:
            ...
            print(
                f"[ERROR] Transformer: Target fingerprint {target_daily_futures_fingerprint[:8]} not found in schema_map. This is unexpected. Skipping."
            )
            continue
        ...
        try:
            ...
            if missing_cols:
                print(
                    f"[WARNING] Transformer: Fingerprint {fingerprint[:8]}... missing columns {missing_cols} during mapping. Filled with NULLs."
                )
            ...
            if not df_to_load.empty:
                ...
                analytics_conn.execute(
                    f"INSERT INTO daily_futures ({', '.join(f'"{c}"' for c in df_to_load.columns)}) SELECT {', '.join(f'"{c}"' for c in df_to_load.columns)} FROM df_to_load"
                )
                transformed_count += 1
            else:
                print(
                    f"[INFO] Transformer: Fingerprint {fingerprint[:8]}... resulted in an empty DataFrame after column mapping. Skipping insert."
                )

        except pd.errors.EmptyDataError:
            print(
                f"[WARNING] Transformer: No data or columns found in CSV for fingerprint {fingerprint[:8]}..."
            )
        except Exception as e:
            print(
                f"[ERROR] Transformer 處理指紋 {fingerprint[:8]}... 的資料時失敗: {e}"
            )
