...
try:
    ...
    print(f"錯誤：模組導入失敗，請確認路徑校正邏輯或依賴是否安裝。錯誤訊息：{e}", file=sys.stderr)
    sys.exit(1)
# --- 樣板結束 ---
...
def main():
    ...
    try:
        ...
        if not finmind_api_token or finmind_api_token == "YOUR_REAL_FINMIND_API_TOKEN_HERE":
            print("錯誤：在 config.yml 中找不到 'finmind' 的 API Token。", file=sys.stderr)
            print("請確認 config.yml 檔案中的 api_keys -> finmind 是否已正確設定。", file=sys.stderr)
            return
        ...
        if investor_data is not None and not investor_data.empty:
            ...
            print("請檢查您的 FinMind API Token 權限、網路連線或確認該日期範圍內有數據。")
