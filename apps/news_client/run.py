...
try:
    ...
    print(
        f"警告 (apps/news_client/run.py __file__): __file__ 未定義，"
        f"專案路徑校正可能不準確。已將 {project_root} 加入 sys.path。",
        file=sys.stderr,
    )
...
from apps.news_client.client import VADER_AVAILABLE, NewsClient  # noqa: E402
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "對抓取到的新聞標題進行情感分析 (使用 VaderSentiment)。"
            f"需要安裝 vaderSentiment 套件。{'目前可用。' if VADER_AVAILABLE else '目前不可用 (套件未安裝)。'}"
        ),
    )
    parser.add_argument(
    ...
    if not final_api_key or "YOUR_" in final_api_key:
        logger.error(
            "NewsClient 的 API Key 未在 config.yml 或命令行參數中有效設定。請檢查 config.yml 中的 'news_client.api_key' 或提供 --newsapi_key 參數。"
        )
        sys.exit(1)

    logger.info(
        f"News client 即將使用 max_retries={max_retries_from_core} (來自核心配置)。"
    )
    ...
    # 目前的 NewsClient 導入似乎有問題，這裡僅為示意
        ...
        logger.error(
            "無法實例化 NewsClient，可能是因為 'apps.news_client.client' 模組不存在或導入失敗。請檢查檔案結構和導入路徑。"
        )
        sys.exit(1)
        ...
        if not news_df.empty:
            ...
            if args.add_sentiment:
                if VADER_AVAILABLE:
                    ...
                    try:
                        df_head_str = (
                            news_df[[args.sentiment_text_column, "vader_compound"]]
                            .head()
                            .to_string()
                        )
                        logger.info(f"\n{df_head_str}")
                    ...
                    logger.warning(
                        "要求進行情感分析，但 VaderSentiment 套件未安裝或不可用。跳過情感分析步驟。"
                    )
