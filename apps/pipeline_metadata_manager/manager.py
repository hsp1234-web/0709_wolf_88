...
class MetadataManager:
    def __init__(
        ...
        default_table_name = core_config.get(
        ...
        self.log_manager.log(
            "DEBUG", f"MetadataManager 使用表格名稱: {self.table_name}"
        )
        ...
        if connection:
            ...
            self.log_manager.log(
                "INFO",
                f"MetadataManager 使用已存在的資料庫連接。DB 路徑: {self.db_path}",
            )
        else:
            default_db_path = core_config.get(
            ...
            self.log_manager.log(
                "INFO", f"MetadataManager 連接到資料庫: {self.db_path}"
            )
    ...
    def _initialize_database(self):
        try:
            ...
            self.log_manager.log(
                "ERROR", f"資料庫表格 '{self.table_name}' 初始化錯誤: {e}"
            )
            raise
    ...
    def write_fingerprint(
        ...
        resolved_etl_version = (
            etl_version
            if etl_version is not None
            else core_config.get(
                "pipeline_metadata_manager.default_etl_version", "unknown"
            )
        )
        self.log_manager.log(
            "DEBUG",
        ...
        try:
            ...
            self.log_manager.log(
                "ERROR", f"寫入指紋 '{fingerprint}' (檔案: {filename}) 時發生錯誤: {e}"
            )
            return False
    ...
    def close(self):
        if hasattr(self, "conn") and self.conn:
            try:
                self.conn.close()
                self.log_manager.log(
                    "INFO", f"資料庫連接已關閉 (路徑: {self.db_path or '外部連接'})。"
                )
            except Exception as e:
...
if __name__ == "__main__":
    ...
    try:
        ...
        manager = MetadataManager(
            log_manager=dummy_logger, db_path=":memory:", table_name="test_table"
        )
        ...
        success = manager.write_fingerprint(
            fingerprint1, TEST_FILE_PATH.name, TEST_FILE_PATH.stat().st_size, "v_test"
        )
