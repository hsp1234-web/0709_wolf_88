...
from __future__ import annotations  # 確保在最頂部
...
def fetch_ohlcv_data_for_assets(assets: list[str], market_db_path: str) -> pd.DataFrame:
    ...
    try:
        ...
        if df.empty:
            print(
                f"警告：在資料庫 {market_db_path} 的 {OHLCV_TABLE_NAME} 表中未找到資產 {assets} 的數據。"
            )
            return pd.DataFrame()
        ...
        print(
            f"成功從 {OHLCV_TABLE_NAME} (來源: {market_db_path}) 為資產 {assets} 提取並轉換了 {len(pivot_df)} 天的數據。"
        )
        return pivot_df
...
def calculate_rolling_correlation(
    price_df: pd.DataFrame, window: int = 30
) -> pd.DataFrame:
    """
    ...
    )  # dropna() 移除第一個 NaN 行 (因 pct_change 產生)
    ...
    for asset1, asset2 in asset_pairs:
        ...
        rolling_corr = (
            returns_df[asset1].rolling(window=window).corr(returns_df[asset2])
        )
        ...
        if not rolling_corr_clean.empty:
            ...
            print(
                f"警告：資產對 {asset1}-{asset2} 在窗口 {window} 下的滾動相關性計算結果為空。"
            )
    ...
    print(
        f"成功計算 {len(asset_pairs)} 個資產對的滾動相關性，共產生 {len(final_corr_df)} 筆相關性數據。"
    )
    return final_corr_df


def store_correlation_data(
    df: pd.DataFrame, analytics_db_path: str, table_name: str = CORRELATION_TABLE_NAME
):
    """
    ...
    try:
        with duckdb.connect(analytics_db_path) as con:  # 使用傳入的 analytics_db_path
            ...
            print(
                f"相關性數據已成功儲存至 DuckDB 資料庫 '{analytics_db_path}' 的資料表 '{table_name}'。"
            )
...
def run_cross_market_correlation_analysis(
    assets_to_analyze: Optional[list[str]] = None,
    window: int = 30,
...
if __name__ == "__main__":
    ...
    try:
        with duckdb.connect(DEFAULT_ANALYTICS_MART_DB) as con:  # 連接到預設分析資料庫
            print(
                f"從 DuckDB 讀取 '{DEFAULT_ANALYTICS_MART_DB}' 的 '{CORRELATION_TABLE_NAME}' 資料表進行驗證..."
            )
            ...
            if CORRELATION_TABLE_NAME not in tables_df["name"].values:
                ...
                print(
                    f"成功從 '{CORRELATION_TABLE_NAME}' 讀取 {len(retrieved_data)} 筆數據。"
                )
