...
class ReportGenerator:
    def __init__(self, db_path: str | Path, log_manager: LogManager):
        ...
        self.log_manager.log(
            "INFO", f"ReportGenerator 初始化，使用資料庫路徑: {self.db_path}"
        )
    ...
    def _validate_timeframe(self, timeframe: str) -> str:
        ...
        if not all(c.isalnum() or c == "_" for c in cleaned_timeframe):
            self.log_manager.log(
                "ERROR", f"Timeframe 驗證失敗: '{timeframe}' 包含無效字符。"
            )
    ...
    def _fetch_data(
        ...
        try:
            ...
            ohlcv_df = con.execute(
                query_ohlcv, [internal_stock_id_for_ohlcv, start_date_str, end_date_str]
            ).fetchdf()
        ...
        if stock_id in ["0050.TW", "TXO"]:
            try:
                ...
                if not pc_ratio_df.empty:
                    ...
                    self.log_manager.log(
                        "INFO",
                        f"成功讀取 {len(pc_ratio_df)} 筆 TXO 的 P/C Ratio 數據。",
                    )
            except Exception as e:
                self.log_manager.log(
                    "WARNING", f"讀取 P/C Ratio (TXO) 數據時發生錯誤: {e}"
                )
    ...
    def _plot_report_plotly(
        ...
        fig = make_subplots(
            rows=3 if has_pc_ratio_data else 2,
            cols=1,
            shared_xaxes=True,
        ...
        fig.add_trace(
            go.Bar(x=ohlcv_df["timestamp"], y=ohlcv_df["volume"], name="成交量"),
            row=2,
            col=1,
        )
        ...
        if chimera_df is not None and not chimera_df.empty:
            ...
            buy_signals = chimera_df[
                chimera_df["composite_signal"].str.contains("買超", na=False)
            ]
            sell_signals = chimera_df[
                chimera_df["composite_signal"].str.contains("賣超", na=False)
            ]
            fig.add_trace(
                go.Scatter(
                    x=buy_signals["date"],
                    y=buy_signals["composite_signal"].apply(
                        lambda s: ohlcv_df["high"].max() * 1.05
                    ),
                    mode="markers",
            ...
            fig.add_trace(
                go.Scatter(
                    x=sell_signals["date"],
                    y=sell_signals["composite_signal"].apply(
                        lambda s: ohlcv_df["high"].max() * 1.05
                    ),
                    mode="markers",
        ...
        fig.update_layout(
            title_text=f"股票 {stock_id} ({timeframe}) 複合信號分析報告",
        ...
        try:
            with self._connect_db() as con:
                ...
                if fig:
                    ...
                    self.log_manager.log(
                        "INFO", f"報告已儲存至: {report_file_path} (HTML 格式)"
                    )
                    return report_file_path
        except Exception as e:
            self.log_manager.log(
                "ERROR", f"生成報告 {stock_id} ({timeframe}) 時發生錯誤: {e}"
            )
        return None
...
if __name__ == "__main__":
    ...
    try:
        ...
        generator.generate_report(
            "TEST_STOCK_D", "2023-01-01", "2023-01-03", "1d", test_output_dir
        )
        generator.generate_report(
            "0050.TW", "2023-01-01", "2023-01-03", "1d", test_output_dir
        )
