    ...
    try:
        with GoldLayerBuilder(db_path=db_path) as builder:
            ...
            for day_offset in range(num_days_sim):
                current_date_base = base_start_datetime + datetime.timedelta(days=day_offset)
                for instrument in instruments:
                    # 每天模擬幾條分鐘線數據
                    price_seed = 100 + day_offset * 0.5 + (5 if instrument == "INSTR_Y" else 0) # 不同的基價和趨勢
            ...
            simulated_silver_df['timestamp'] = pd.to_datetime(simulated_silver_df['timestamp'])
            print(f"[INFO] 成功創建 {len(simulated_silver_df)} 筆模擬銀層分鐘數據 (涵蓋 {num_days_sim} 天, {len(instruments)} 個標的)。")
            # print("[DEBUG] 模擬銀層 DataFrame (部分):")
            # print(simulated_silver_df.sample(5))

            # 2. 調用 aggregate_to_daily_ohlcv
            print("\n[INFO] 步驟 2: 正在將分鐘數據聚合為日線 OHLCV...")
            daily_ohlcv_df = builder.aggregate_to_daily_ohlcv(simulated_silver_df.copy())
            ...
            if features_inclusive_df.empty: # 應該不會發生，因為 daily_ohlcv_df 非空
                print("[WARN] 特徵計算結果為空。")
                return
            print(f"[INFO] 成功計算技術特徵，最終 DataFrame 包含 {len(features_inclusive_df)} 筆記錄。")
            ...
            print(f"[INFO] 數據已成功存入金層資料表: '{gold_ohlcv_table}' 和 '{gold_features_table}'。")
