...
from pathlib import Path # 確保 Path 在此導入，因為 if __name__ == '__main__' 中使用
import logging
...
if __name__ == '__main__':
    ...
    cli_logger.setLevel(logging.DEBUG) # 設置為 DEBUG 以查看詳細日誌
    ...
    # 數據3 (id, value) - 更多數據，部分與 df1 重疊 id
    ...
    # 測試3: 再次載入 df1 (冪等性，不應增加數據)
    cli_logger.info(f"\n--- 測試3: 再次載入 {parquet_file_1} (冪等性測試) ---")
    load_parquet_to_db(str(parquet_file_1), str(db_file), table, primary_key_column='id', logger=cli_logger)

    # 測試4: 載入 df3 (包含部分重疊 ID 和新 ID)
    ...
    # 清理臨時檔案
    # parquet_file_1.unlink(missing_ok=True)
    # parquet_file_2_schema_diff.unlink(missing_ok=True)
    # parquet_file_3_more_data.unlink(missing_ok=True)
    # db_file.unlink(missing_ok=True)
    # test_dir.rmdir() # 只有在目錄為空時才能成功
    # cli_logger.info(f"建議手動清理目錄: {test_dir.resolve()}")
