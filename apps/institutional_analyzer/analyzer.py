...
import duckdb
import pandas as pd
from pathlib import Path  # 添加 Path 導入
from core.clients.finmind import FinMindClient  # 更新為 FinMindClient
...
project_root_dir = Path(os.getcwd())  # 假設執行時 CWD 是專案根目錄
DB_PATH = project_root_dir / "analytics_mart.duckdb"


class InstitutionalAnalyzer:
    def __init__(
        self, stock_id: str, start_date: str, end_date: str, api_token: Optional[str] = None
    ):
        self.stock_id = stock_id
        self.start_date = start_date
        self.end_date = end_date
        self.fm_client = FinMindClient(api_token=api_token)  # 確認實例化的類名已更新
    ...
    def fetch_data(self) -> pd.DataFrame | None:
        ...
        print(
            f"正在從 FinMind API 獲取股票 {self.stock_id} 從 {self.start_date} 到 {self.end_date} 的三大法人買賣超數據..."
        )
        try:
            ...
            print(
                f"調用 FinMind API (get_taiwan_stock_institutional_investors_buy_sell) 時發生錯誤: {e}"
            )
            return None
    ...
    def analyze_data(self, data: pd.DataFrame) -> pd.DataFrame | None:
        ...
        processed_data["net_shares"] = (
            processed_data["buy_shares"] - processed_data["sell_shares"]
        )
        ...
        final_data = processed_data.groupby(
            ["date", "stock_id", "investor_type"], as_index=False
    ...
    def store_results(self, analyzed_data: pd.DataFrame):
        ...
        try:
            ...
            con.close()
            print(
                f"成功將 {len(analyzed_data)} 筆結果儲存/更新至 institutional_trades 表格。"
            )
        except Exception as e:
    ...
    def run_analysis(self):
        """執行完整的數據獲取、分析與儲存流程"""
        print(
            f"開始為股票 {self.stock_id} 執行法人籌碼分析 ({self.start_date} 至 {self.end_date})..."
        )
...
if __name__ == "__main__":
    ...
    if not api_token:
        print(
            "警告：FINMIND_API_TOKEN 環境變數未設定。FinMindClient 將無法獲取真實數據，僅能進行初始化。"
        )
    ...
    try:
        ...
        if (
            api_token != "DUMMY_TOKEN_FOR_TESTING_ANALYZER"
        ):  # 只在有真實 token 時才預期有數據
            print("\n--- 初步數據庫驗證 ---")
            try:
                con = duckdb.connect(DB_PATH, read_only=True)
                result_df = con.execute(
                    f"SELECT * FROM institutional_trades WHERE stock_id = '{test_stock}' AND date >= '{test_start}' AND date <= '{test_end}' ORDER BY date, investor_type"
                ).fetchdf()
                if not result_df.empty:
                    print(
                        f"在 institutional_trades 表中找到 {len(result_df)} 筆關於 {test_stock} 的測試數據:"
                    )
                    print(result_df.head())
                else:
                    print(
                        f"未在 institutional_trades 表中找到關於 {test_stock} 的測試數據。這可能是因為 API 未返回數據，或 API Token 無效/額度用盡。"
                    )
                con.close()
            except Exception as e:
