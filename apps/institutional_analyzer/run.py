...
try:
    ...
    print(
        f"專案路徑校正時發生錯誤 (apps/institutional_analyzer/run.py): {e}",
        file=sys.stderr,
    )
# --- 標準化「路徑自我校正」樣板碼 END ---
...
def main():
    ...
    parser.add_argument(
        "--stock-id", required=True, type=str, help="要分析的股票代碼 (例如 2330)。"
    )
    parser.add_argument(
        "--start-date", required=True, type=str, help="查詢開始日期 (YYYY-MM-DD)。"
    )
    parser.add_argument(
        "--end-date", required=True, type=str, help="查詢結束日期 (YYYY-MM-DD)。"
    )
    parser.add_argument(
        "--api-token",
        type=str,
        default=os.getenv("FINMIND_API_TOKEN"),
        help="您的 FinMind API Token (可選，預設從 FINMIND_API_TOKEN 環境變數讀取)。",
    )

    args = parser.parse_args()

    print("指令：執行機構法人買賣超分析...")
    ...
    if args.api_token:
        ...
        print(
            "API Token: 未提供，將依賴 FinMindClient 的內部邏輯 (可能使用環境變數或報錯)"
        )
    ...
    if not token_to_use:
        ...
        print(
            "提示：未通過 --api-token 參數提供 API token，將依賴 FINMIND_API_TOKEN 環境變數。"
        )
    ...
    try:
        ...
        print(
            f"機構法人買賣超分析 ({args.stock_id}, {args.start_date} 至 {args.end_date}) 執行完畢。"
        )
