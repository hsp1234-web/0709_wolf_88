...
class TimeAggregator:
    ...
    def read_bronze_ticks(
        self,
        start_time: datetime.datetime,
        end_time: datetime.datetime,
        bronze_table_name: str = "bronze_taifex_ticks",
    ) -> pd.DataFrame:
        """
    ...
    def aggregate_to_1m_ohlcv(self, ticks_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if (
            "timestamp" not in ticks_df.columns
            or not pd.api.types.is_datetime64_any_dtype(ticks_df["timestamp"])
        ):
            raise ValueError(
                "輸入的 DataFrame 必須包含 'timestamp' 欄位且其類型應為 datetime-like。"
            )
        ...
        for instrument, group_df in ticks_df.groupby("instrument"):
            ...
            # 合併價格和成交量
            ohlcv_instrument_df = pd.concat(
                [price_resampled, volume_resampled.rename("volume")], axis=1
            )
    ...
    def write_silver_ohlcv(
        self, ohlcv_df: pd.DataFrame, silver_table_name: str = "silver_market_ohlcv_1m"
    ):
        """
    ...
    try:
        with TimeAggregator(db_path=test_db_path) as aggregator:
            ...
            mock_ticks_data.extend(
                [
                    (
                        start_dt + datetime.timedelta(seconds=10),
                        16700.0,
                        5,
                        "TXF202310",
                    ),
                    ...
                    (
                        start_dt + datetime.timedelta(seconds=55),
                        16701.0,
                        1,
                        "TXF202310",
                    ),  # Close for 1st min
                    (
                        start_dt + datetime.timedelta(minutes=1, seconds=5),
                        16705.0,
                        4,
                        "TXF202310",
                    ),  # Open for 2nd min
                    (
                        start_dt + datetime.timedelta(minutes=1, seconds=15),
                        16706.0,
                        6,
                        "TXF202310",
                    ),
            ...
            mock_ticks_data.extend(
                [
                    (
                        start_dt + datetime.timedelta(seconds=15),
                        3340.0,
                        10,
                        "MXF202310",
                    ),
                    (
                        start_dt + datetime.timedelta(seconds=45),
                        3345.0,
                        12,
                        "MXF202310",
                    ),
            ...
            conn.register(
                ...
                pd.DataFrame(
                    mock_ticks_data,
                    columns=["timestamp", "price", "volume", "instrument"],
                ),
            )
            conn.execute(f"INSERT INTO {bronze_table} SELECT * FROM mock_ticks_df_view")
            conn.unregister("mock_ticks_df_view")
            print(
                f"[Test Setup] 成功插入 {len(mock_ticks_data)} 筆假 Tick 數據到 '{bronze_table}'。"
            )

            # 2. 測試 read_bronze_ticks
            print("\n[Test Read] 正在測試 read_bronze_ticks...")
            query_start_time = start_dt
            query_end_time = start_dt + datetime.timedelta(
                minutes=2
            )  # 包含第二分鐘的開始
            read_ticks_df = aggregator.read_bronze_ticks(
                query_start_time, query_end_time, bronze_table_name=bronze_table
            )
            ...
            txf_first_min = ohlcv_df[
                (ohlcv_df["instrument"] == "TXF202310")
                & (ohlcv_df["timestamp"] == pd.Timestamp(start_dt))
            ]
            ...
            # 驗證數據是否已寫入銀層表
            silver_count_result = conn.execute(
                f"SELECT COUNT(*) FROM {silver_table}"
            ).fetchone()
            if silver_count_result:
                print(
                    f"[Test Write] 銀層資料表 '{silver_table}' 中的記錄數: {silver_count_result[0]}"
                )
            ...
            print("\n[Test Write] 銀層數據預覽:")
            silver_data_preview = conn.execute(
                f"SELECT * FROM {silver_table} ORDER BY instrument, timestamp"
            ).fetchdf()
