...
if __name__ == '__main__':
    # 簡易的命令行測試 (非正式測試)
    # python -m apps.taifex_data_transformer.transformer --zipfile /path/to/your.zip --output /path/to/output_dir
    import argparse
    parser = argparse.ArgumentParser(description="TAIFEX Data Transformer CLI (Test Only)")
    ...
    cli_logger = logging.getLogger("CLI_TEST")
    ...
    cli_logger.info(f"手動測試: 處理 ZIP 檔案 '{args.zipfile}', 輸出到 '{args.output}'")
    process_zip_file(args.zipfile, args.output, logger=cli_logger)
    cli_logger.info("手動測試完成。")
