...
def _process_zip_file_internal(zip_path_str: str, output_dir_str: str, logger: logging.Logger) -> bool:
    """
    ...
    try:
        with zipfile.ZipFile(zip_path, 'r') as zf:
            ...
            if target_csv_info is None:
                logger.error(f"[ERROR] 情報包內容與預期不符，未找到目標 CSV 檔案。檔案路徑: {zip_path}")
                return False
            ...
            try:
                ...
                logger.error(f"[ERROR] 情報編碼錯誤，無法使用標準 UTF-8 解碼。檔案路徑: {zip_path}, CSV 檔案: {csv_filename}。錯誤: {ude}")
                return False

            if decoded_csv_string is None:
                logger.error(f"[ERROR] 未知原因導致 CSV 字串未能成功解碼。檔案路徑: {zip_path}")
                return False
            ...
            try:
                ...
                logger.error(f"[ERROR] 情報內部格式錯亂或解析錯誤。檔案路徑: {zip_path}, CSV 檔案: {csv_filename}。錯誤: {pe}")
                return False
            except Exception as e_parse:
                logger.error(f"[ERROR] 解析 CSV 時發生未預期錯誤。檔案路徑: {zip_path}, CSV 檔案: {csv_filename}。錯誤: {e_parse}")
                return False

            if df is None:
                 logger.error(f"[ERROR] DataFrame 未能生成，原因未知。檔案路徑: {zip_path}, CSV 檔案: {csv_filename}")
                 return False

            if df.empty:
                logger.warning(f"[WARNING] 從 '{csv_filename}' 讀取的 DataFrame 為空。檔案路徑: {zip_path}。")
            ...
            try:
                ...
                logger.info(f"成功將 '{csv_filename}' 從 '{zip_path.name}' 轉換並儲存為 '{output_parquet_path}'")
                return True
            except Exception as e_to_parquet:
                logger.error(f"[ERROR] 將 DataFrame 儲存為 Parquet 時發生錯誤。檔案路徑: {zip_path}, CSV 檔案: {csv_filename}。錯誤: {e_to_parquet}")
                return False
        ...
        logger.error(f"[ERROR] 處理 ZIP 檔案時發生未預期錯誤。檔案路徑: {zip_path}。錯誤: {e}")
        return False
...
def run_transformation(argv: Optional[List[str]] = None) -> bool:
    ...
    """
    parser = argparse.ArgumentParser(description="TAIFEX Data Transformation Module (ETL Pipeline)")
    ...
    if not current_logger.handlers:
        ...
        current_logger.info(f"為 transformer 模組 logger 新增了 StreamHandler，等級設為 {args.loglevel.upper()}")


    current_logger.info(f"ETL Transformer: 處理 ZIP 檔案 '{args.zipfile}', 輸出到 '{args.output}'")

    success = _process_zip_file_internal(args.zipfile, args.output, logger=current_logger)

    if success:
        current_logger.info(f"ETL Transformer: 任務成功完成。ZIP: '{args.zipfile}' -> Parquet Dir: '{args.output}'")
    else:
