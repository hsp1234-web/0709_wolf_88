...
def _get_parquet_schema(
    parquet_path: str, logger: logging.Logger
) -> List[Dict[str, str]]:
    ...
    try:
        ...
        if not schema_info:
            logger.warning(
                f"無法從 Parquet 檔案 {parquet_path} 中獲取 schema (DESCRIBE 結果為空)。"
            )
            return []
        schema = [
            {"name": str(col[0]), "type": str(col[1]).upper()} for col in schema_info
        ]
...
def _get_table_schema_from_db(
    db_conn: duckdb.DuckDBPyConnection, table_name: str, logger: logging.Logger
    ...
    try:
        ...
        if not schema_info:
            logger.warning(
                f"無法從資料庫中獲取表格 '{table_name}' 的 schema (PRAGMA table_info 結果為空)。"
            )
            return None
        schema = [
            {"name": str(col[1]), "type": str(col[2]).upper()} for col in schema_info
        ]
...
def _compare_schemas(
    ...
    if not parquet_schema or not table_schema:
        logger.warning(
            f"Schema 不匹配：其中一個 schema 為空。Parquet empty: {not parquet_schema}, Table empty: {not table_schema}"
        )
        return False
    if len(parquet_schema) != len(table_schema):
        logger.warning(
            f"Schema 不匹配：欄位數量不同。Parquet: {len(parquet_schema)}, Table: {len(table_schema)}"
        )
        return False
    for i in range(len(parquet_schema)):
        ...
        if pq_col["name"].lower() != tbl_col["name"].lower():
            logger.warning(
                f"Schema 不匹配：欄位名稱在位置 {i} 不同。Parquet: '{pq_col['name']}', Table: '{tbl_col['name']}'"
            )
            return False
...
def _load_parquet_to_db_internal(  # Renamed from load_parquet_to_db
    ...
    effective_logger = (
        logger if logger else MODULE_LOGGER
    )  # Use module logger if specific one isn't passed
    parquet_path = Path(parquet_path_str)

    if not parquet_path.exists() or not parquet_path.is_file():
        effective_logger.error(
            f"Parquet 檔案不存在或不是一個檔案。檔案路徑: {parquet_path}"
        )
        return False
    ...
    try:
        effective_logger.info(
            f"準備將 Parquet 檔案 '{parquet_path.name}' 載入到資料庫 '{db_path_str}' 的表格 '{table_name}'。"
        )
        # 確保資料庫目錄存在
        ...
        if not pq_schema:
            effective_logger.error(
                f"無法確定 Parquet 檔案 '{parquet_path.name}' 的 schema，終止載入。"
            )
            if db_conn:
                db_conn.close()
            return False

        db_table_schema = _get_table_schema_from_db(
            db_conn, table_name, effective_logger
        )

        if db_table_schema is None:
            effective_logger.info(
                f"表格 '{table_name}' 不存在，將根據 Parquet 檔案 schema 創建。"
            )
            db_conn.execute(
                f"CREATE TABLE \"{table_name}\" AS SELECT * FROM read_parquet('{str(parquet_path)}')"
            )
            effective_logger.info(
                f"成功創建表格 '{table_name}' 並從 '{parquet_path.name}' 載入數據。"
            )
        else:
            effective_logger.info(
                f"表格 '{table_name}' 已存在。將進行 schema 驗證和冪等載入。"
            )
            ...
            if primary_key_column:
                if not any(
                    col["name"].lower() == primary_key_column.lower()
                    for col in pq_schema
                ):
                    effective_logger.error(
                        f"提供的主鍵欄位 '{primary_key_column}' 不存在於 Parquet。執行簡單追加。"
                    )
                    db_conn.execute(
                    ...
                    effective_logger.info(
                        f"使用主鍵 '{primary_key_column}' 進行冪等載入。暫存表: '{staging_table_name}'。"
                    )
                    db_conn.execute(
                        f"CREATE TEMP TABLE \"{staging_table_name}\" AS SELECT * FROM read_parquet('{str(parquet_path)}')"
                    )
                    ...
                    effective_logger.info("冪等載入完成。")
            else:
                effective_logger.info(
                    f"未提供主鍵。將 '{parquet_path.name}' 中的數據追加到 '{table_name}'。"
                )
                db_conn.execute(
                    f"INSERT INTO \"{table_name}\" SELECT * FROM read_parquet('{str(parquet_path)}')"
                )
        ...
        effective_logger.error(
            f"[ERROR] 資料庫 '{db_path_str}' I/O 錯誤或鎖定: {e_lock}"
        )
        return False
    except (
        SchemaMismatchError
    ) as e_schema:  # Caught from _compare_schemas or raised directly
        ...
        effective_logger.error(
            f"載入 Parquet '{parquet_path.name}' 時發生未預期錯誤: {e}", exc_info=True
        )
        return False
    finally:
        if db_conn is not None:
            try:
                ...
                effective_logger.error(
                    f"關閉資料庫連接 '{db_path_str}' 時發生錯誤: {e_close}"
                )
...
def run_loading(argv: Optional[List[str]] = None) -> bool:
    ...
    parser = argparse.ArgumentParser(
        description="Database Loading Module (ETL Pipeline)"
    )
    parser.add_argument(
        "--parquet-file", required=True, help="Path to the input Parquet file."
    )
    parser.add_argument(
        "--db-path", required=True, help="Path to the DuckDB database file."
    )
    parser.add_argument(
        "--table-name", required=True, help="Target table name in the database."
    )
    parser.add_argument(
        "--primary-key", help="Primary key column for idempotent loading."
    )
    parser.add_argument(
        "--loglevel",
        default="INFO",
    ...
    if (
        not current_logger.handlers
    ):  # Ensure handler for direct calls or specific environments
        ...
        current_logger.info(
            f"為 loader 模組 logger 新增了 StreamHandler，等級設為 {args.loglevel.upper()}"
        )

    current_logger.info(
        f"ETL Loader: 載入 Parquet '{args.parquet_file}' 到 DB '{args.db_path}', 表 '{args.table_name}'。"
    )
    ...
    try:
        ...
        if success:
            current_logger.info(
                f"ETL Loader: 任務成功完成。Parquet: '{args.parquet_file}' -> DB: '{args.db_path}', Table: '{args.table_name}'"
            )
        else:
            # _load_parquet_to_db_internal should have logged the specific error
            current_logger.error(
                f"ETL Loader: 任務失敗。Parquet: '{args.parquet_file}'"
            )
        ...
        current_logger.error(
            "ETL Loader: 任務因 Schema 不匹配而失敗 (捕獲於 run_loading)。"
        )
        success = False  # Ensure success is False
    except Exception as e:
        current_logger.error(
            f"ETL Loader: 任務因未預期錯誤而失敗 (捕獲於 run_loading): {e}",
            exc_info=True,
        )
        success = False  # Ensure success is False
