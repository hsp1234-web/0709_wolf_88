...
def create_ohlcv_tables(con: duckdb.DuckDBPyConnection, analytics_db_name_for_log: str):
    ...
    for period_name in TIME_PERIODS.keys():
        ...
        try:
            ...
            print(
                f"在 {analytics_db_name_for_log} 中創建資料表 '{table_name}' 時發生錯誤: {e}"
            )
            raise
    ...
    try:
        with duckdb.connect(
            ...
            with duckdb.connect(
                ...
                ORDER BY timestamp;
                """
                ticks_df = source_con.execute(
                    query, [product_id, start_date_str, end_date_str]
                ).fetchdf()
                ...
                for period_name, period_code in TIME_PERIODS.items():
                    ...
                    ohlcv_df = resampled_data.dropna(
                    ...
                    if not ohlcv_df.empty:
                        ...
                        analytics_con.execute(
                            delete_query,
                            [product_id, min_ts_to_delete, max_ts_to_delete],
                        )
...
def _create_dummy_source_db_if_needed(db_path: Path, product_id_for_dummy: str):
    if not db_path.exists():
        print(
            f"警告：指定的來源資料庫 {db_path} 不存在。將創建一個包含範例數據的假資料庫。"
        )
        _ensure_db_directory_exists(db_path)
        try:
            with duckdb.connect(str(db_path)) as con:
                con.execute(
                    "CREATE TABLE IF NOT EXISTS ticks (timestamp TIMESTAMP, product_id VARCHAR, price DOUBLE, qty BIGINT);"
                )
                sample_data = [
...
def run_aggregation(argv: list[str] | None = None):
    ...
    parser.add_argument(
        "product_id", type=str, help="要處理的商品代碼 (例如：MXF1 或 2330)。"
    )
    parser.add_argument(
        "start_date", type=str, help="開始日期 (YYYY-MM-DD HH:MM:SS 或 YYYY-MM-DD)。"
    )
    parser.add_argument(
        "end_date",
        type=str,
        help="結束日期 (YYYY-MM-DD HH:MM:SS 或 YYYY-MM-DD) (不包含此日/時間點)。",
    )
    parser.add_argument(
        "--source_db",
        type=str,
        default=str(cfg_source_ticks_db_path),
        help="來源 Tick 資料庫路徑。",
    )
    parser.add_argument(
        "--analytics_db",
        type=str,
        default=str(cfg_analytics_db_path),
        help="分析結果資料庫路徑。",
    )
    ...
    print(
        f"--- 時間聚合模組：商品 {args.product_id}，從 {args.start_date} 到 {args.end_date} ---"
    )
    ...
    if (
        args.source_db == str(DEFAULT_SOURCE_TICKS_DB_PATH)
        and not Path(args.source_db).exists()
    ):
        try:
