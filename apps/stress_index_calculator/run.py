...
try:
    ...
    if 'project_root' not in locals() and isinstance(e, NameError):
        ...
        if (project_root.name == 'stress_index_calculator' and project_root.parent.name == 'apps'):
...
from apps.stress_index_calculator.calculator import calculate_derived_indicators, calculate_stress_index, calculate_macd_momentum
...
logger = setup_logger('stress_index_calculator', level=logging.DEBUG) # 設定日誌級別為 DEBUG

def fetch_source_data(db_path: Path, start_date_str: str, end_date_str: str) -> pd.DataFrame: # db_path 類型改為 Path
    """從來源資料庫提取所有需要的真實數據並合併"""
    db_path_str = str(db_path) # 轉換為字串供日誌和 os.path.exists 使用
    logger.info(f"正在從 {db_path_str} 提取真實數據，日期範圍: {start_date_str} 至 {end_date_str}...")
    ...
    try:
        with duckdb.connect(database=db_path_str, read_only=True) as con: # 確保傳遞字串
            # 建立一個包含所有交易日的基礎日期表
            ...
            if gspc_count_result is None or gspc_count_result[0] == 0:
                logger.error(f"交易日曆基準 (^GSPC) 在指定日期範圍 {start_date_str} 至 {end_date_str} 內無數據。")
            ...
            print(f"DEBUG SQL QUERY STRING: ---{trading_days_view_query}---") # Direct print
            ...
            if fred_df_raw.empty:
                ...
                fred_df_pivot = pd.DataFrame(columns=['SOFR', 'DGS10', 'DGS2', 'VIX', 'Reserves'])
            else:
                fred_df_pivot = fred_df_raw.pivot(index='date_col', columns='series_id', values='value')
                fred_df_pivot.rename(columns={'VIXCLS': 'VIX', 'WRESBAL': 'Reserves'}, inplace=True)
            logger.info(f"FRED 數據提取完成，原始數據 {len(fred_df_raw)} 筆, Pivot 後 {len(fred_df_pivot)} 筆。")
            ...
            # 確保欄位為數值
            cols_to_convert = ['SOFR', 'DGS10', 'DGS2', 'VIX', 'Reserves',
            ...
            logger.info(f"成功提取並合併 {len(final_df)} 筆數據。最終欄位: {final_df.columns.tolist()}")
        ...
        logger.error(f"涉及的查詢可能包含錯誤的表名、欄位名，或日期範圍 '{start_date_str}' 至 '{end_date_str}' 內無有效數據。")
        return pd.DataFrame()
    except Exception as e:
        logger.error(f"提取真實數據時發生未預期錯誤: {e}", exc_info=True)
        return pd.DataFrame()

def save_results(df: pd.DataFrame, db_path: Path, table_name: str): # db_path 類型改為 Path
    """將計算結果儲存到目標資料庫"""
    ...
    logger.info(f"正在將 {len(df)} 筆計算結果儲存至 {db_path_str} 的 {table_name} 表...")
    try:
        with duckdb.connect(database=db_path_str, read_only=False) as con: # 確保傳遞字串
            # 確保索引 (日期) 被儲存為一個常規列
            df_to_save = df.reset_index()
            con.register('result_df', df_to_save)
            # 確保日期欄位名稱正確，假設索引名是 'date'
            if 'date' not in df_to_save.columns and 'index' in df_to_save.columns and pd.api.types.is_datetime64_any_dtype(df_to_save['index']):
...
def main():
    ...
    parser.add_argument('--start_date', type=str, default=default_start, help="計算開始日期 (YYYY-MM-DD)")
    parser.add_argument('--end_date', type=str, default=default_end, help="計算結束日期 (YYYY-MM-DD)")
    ...
    config = {
        ...
        'macd_params': {'fast': 12, 'slow': 26, 'signal': 9}, # 用於 calculate_macd_momentum
        'macd_colors': {'blue': "#6495ED", 'green': "#3CB371", 'red': "#B22222"} # 用於 calculate_macd_momentum
    }
    ...
    df_stress = calculate_stress_index(df_derived, config) # config 應只包含 calculate_stress_index 所需的參數
    ...
    final_df = calculate_macd_momentum(df_stress, config) # config 應只包含 calculate_macd_momentum 所需的參數
