...
class DBManager:
    ...
    def __init__(
        self,
        db_path: str,
        target_ohlcv_table_name: str = "market_ohlcv_data",
        ...
        self.duckdb_config = (
            duckdb_config if duckdb_config else {}
        )  # 確保是字典以便合併
        ...
        config_info = (
            f"使用配置: {self.duckdb_config}" if self.duckdb_config else "使用預設配置"
        )
        print(
            f"INFO: DBManager (Daily Market Analyzer v33.0) 初始化完畢，資料庫路徑: {self.db_path}。{config_info}"
        )
    ...
    def _setup_database(self):
        ...
        try:
            with duckdb.connect(
                database=self.db_path, config=self.duckdb_config
            ) as con:
                # 建立 OHLCV 資料表
                create_ohlcv_sql = f"""
                ...
                print(
                    f"INFO: 資料表 '{self.ohlcv_table_name}' 已在資料庫 '{self.db_path}' 中準備就緒。"
                )

                # 建立 no_data_records 資料表
                create_no_data_records_sql = """
                ...
                print(
                    f"INFO: 資料表 'no_data_records' 已在資料庫 '{self.db_path}' 中準備就緒。"
                )

                # 建立 FactorStore_Daily 資料表
                create_factor_store_sql = """
                ...
                print(
                    f"INFO: 資料表 'FactorStore_Daily' 已在資料庫 '{self.db_path}' 中準備就緒。"
                )

                # 建立 StrategicDashboard_Daily 資料表
                create_strategic_dashboard_sql = """
                ...
                print(
                    f"INFO: 資料表 'StrategicDashboard_Daily' 已在資料庫 '{self.db_path}' 中準備就緒。"
                )
    ...
    def create_ohlcv_table(self, table_name: str | None = None):
        ...
        **注意：此方法在新版中主要由 _setup_database 處理，保留可能是為了向後兼容或特定測試。**
        ...
        effective_table_name = (
            table_name if table_name is not None else self.ohlcv_table_name
        )

        if (
            effective_table_name != self.ohlcv_table_name
        ):  # 此警告邏輯可能需要調整或移除，因為_setup_database已處理預設表
            print(
                f"警告: create_ohlcv_table 被呼叫以建立一個與 DBManager 預設 ({self.ohlcv_table_name}) 不同的資料表: {effective_table_name}。"
            )
        ...
        try:
            ...
            print(
                f"INFO: (create_ohlcv_table) 資料表 '{table_name}' 已在資料庫 '{self.db_path}' 中準備就緒 (包含 interval 欄位)。"
            )
        except Exception as e:
    ...
    def upsert_data(self, df: pd.DataFrame, table_name: str):
        ...
        if df.empty:
            ...
            print(
                f"INFO: 傳入的 DataFrame ({current_ticker}) 為空，無需寫入資料表 '{table_name}'。"
            )
            return
        ...
        if "index" in df_to_insert.columns and "datetime" not in df_to_insert.columns:
            ...
            print("資訊 (DBManager): 將來自索引的 'index' 欄位重命名為 'datetime'。")

        if "datetime" not in df_to_insert.columns and "date" in df_to_insert.columns:
            print(
                "警告 (DBManager): DataFrame 中缺少 'datetime' 欄位，但找到了 'date' 欄位。將自動重命名 'date' 為 'datetime'。建議上游模組應直接提供 'datetime' 欄位。"
            )
        ...
        if missing_cols:
            current_ticker_for_error = (
                df_to_insert["ticker"].iloc[0]
                if "ticker" in df_to_insert.columns and not df_to_insert.empty
                else "未知 Ticker"
            )
            print(
                f"錯誤: DataFrame ({current_ticker_for_error}) 缺少必要欄位: {', '.join(missing_cols)}。無法寫入資料表 '{table_name}'。"
            )
            df_to_insert.info()
            return
        ...
        try:
            ...
            if df_to_insert["datetime"].dt.tz is None:
                df_to_insert["datetime"] = df_to_insert["datetime"].dt.tz_localize(
                    "UTC"
                )
            else:
        ...
        try:
            ...
            print(
                f"INFO: 成功將 {len(df_to_insert)} 筆來自 '{current_ticker}' (顆粒度: {current_interval}) 的數據寫入/更新至資料表 '{table_name}'。"
            )
        except Exception as e:
            current_ticker_for_error = (
                df_to_insert["ticker"].iloc[0]
            ...
            print(
                f"錯誤: 寫入數據到資料表 '{table_name}' 失敗 (Ticker: {current_ticker_for_error}): {e}"
            )
            print(f"DEBUG: 嘗試寫入的 DataFrame ({current_ticker_for_error}) info:")
            df_to_insert.info()

    def query_data_for_day(
        self, ticker: str, date_str: str, table_name: str | None = None
    ) -> pd.DataFrame:
        effective_table_name = (
            table_name if table_name is not None else self.ohlcv_table_name
        )
        try:
            ...
            start_of_next_day = (target_date + timedelta(days=1)).strftime(
                "%Y-%m-%d 00:00:00"
            )
            query = f"""
            ...
            with duckdb.connect(
                ...
                result_df = con.execute(
                    query, [ticker, start_of_day, start_of_next_day]
                ).fetchdf()
    ...
    def query_previous_day_close(
        ...
        try:
            ...
            for i in range(1, max_lookback_days + 1):
                ...
                daily_data_df = self.query_data_for_day(
                    ticker, prev_date_to_check_str, effective_table_name
                )
    ...
    def check_cache(
        ...
        print(
            f"DEBUG: check_cache: ticker={ticker}, start={start_date_str}, end={end_date_str}, interval={interval}, table={effective_table_name}"
        )
        ...
        try:
            ...
            if not result_df.empty and "datetime" in result_df.columns:
                ...
                print(
                    f"DEBUG: check_cache: Requested dates set: {sorted(list(requested_dates_set))}"
                )
                print(
                    f"DEBUG: check_cache: Cached dates set for {ticker} ({interval}): {sorted(list(cached_dates_set))}"
                )
                ...
                print(
                    f"DEBUG: check_cache: No data found in cache for {ticker} ({interval}) in range. All requested dates are missing."
                )
        except Exception as e:
            print(
                f"錯誤 (check_cache): 查詢快取數據失敗 for {ticker} ({interval}): {e}"
            )
            missing_dates = sorted(list(requested_dates_set))
            cached_df = pd.DataFrame()
        print(
    ...
    def check_data_exists_for_date_and_ticker(
        self, ticker: str, date_str: str, table_name: str | None = None
    ) -> bool:
        ...
        try:
            ...
            query_end_ts = (target_date + timedelta(days=1)).strftime(
                "%Y-%m-%d 00:00:00"
            )
            ...
            with duckdb.connect(
                ...
                count = con.execute(
                    query, [ticker, query_start_ts, query_end_ts]
                ).fetchone()[0]

            exists = count > 0
            print(
                f"DEBUG [DB_DATA_EXISTS_CHECK]: Ticker: {ticker}, Date: {date_str}, Table: {effective_table_name}. Data exists: {exists} (Count: {count})"
            )
            return exists
        except Exception as e:
            print(
                f"ERROR [DB_DATA_EXISTS_CHECK]: Failed for Ticker: {ticker}, Date: {date_str}, Table: {effective_table_name}. Error: {e}"
            )
            return False  # 出錯時保守假設不存在，以便上游重新獲取

    def record_no_data_range(
        ...
        try:
            ...
            print(
                f"INFO: 已記錄/更新無數據區塊: Ticker={ticker}, Interval={interval}, Range=[{start_date} - {end_date}], RecordedAt={recorded_at}"
            )
        except Exception as e:
            print(
                f"錯誤: 記錄無數據區塊失敗: Ticker={ticker}, Interval={interval}, Range=[{start_date} - {end_date}]: {e}"
            )

    def check_no_data_record_exists(
        self,
        ticker: str,
        interval: str,
        start_date: str,
        ...
        try:
            with duckdb.connect(
                ...
                result = con.execute(
                    sql,
                    [ticker, interval, start_date, end_date, threshold_timestamp_str],
                ).fetchone()
            if result and result[0] > 0:
                print(
                    f"DEBUG: 發現有效的無數據記錄: Ticker={ticker}, Interval={interval}, Range=[{start_date} - {end_date}], Cooldown={cooldown_days} days."
                )
                return True
            return False
        except Exception as e:
            print(
                f"錯誤: 檢查無數據記錄失敗: Ticker={ticker}, Interval={interval}, Range=[{start_date} - {end_date}]: {e}"
            )
    ...
    def insert_factors(self, df: pd.DataFrame):
        ...
        if df.empty:
            print(
                "INFO (DBManager - insert_factors): 傳入的因子 DataFrame 為空，無需寫入。"
            )
            return
        ...
        if missing_cols:
            print(
                f"錯誤 (DBManager - insert_factors): DataFrame 缺少必要欄位: {', '.join(missing_cols)}。無法寫入 FactorStore_Daily。"
            )
            df_to_insert.info()
            return
        ...
        try:
            ...
            if pd.api.types.is_datetime64_any_dtype(df_to_insert["date"]):
                ...
                df_to_insert["date"] = pd.to_datetime(df_to_insert["date"]).dt.strftime(
                    "%Y-%m-%d"
                )

            df_to_insert["ticker"] = df_to_insert["ticker"].astype(str)
            df_to_insert["factor_name"] = df_to_insert["factor_name"].astype(str)
            df_to_insert["factor_value"] = pd.to_numeric(
                df_to_insert["factor_value"], errors="raise"
            )  # 確保是數值型態
        ...
        try:
            ...
            print(
                f"INFO (DBManager - insert_factors): 成功將 {num_rows} 筆因子數據 ({unique_tickers} 個 tickers, {unique_factor_names} 個因子名) 寫入/更新至 '{table_name}'。"
            )

        except Exception as e:
            print(
                f"錯誤 (DBManager - insert_factors): 寫入因子數據到資料表 '{table_name}' 失敗: {e}"
            )
            print("DEBUG: 嘗試寫入的因子 DataFrame (前5行):")
    ...
    def insert_strategic_signals(self, signals_df: pd.DataFrame):
        ...
        if signals_df.empty:
            print(
                "INFO (DBManager - insert_strategic_signals): 傳入的信號 DataFrame 為空，無需寫入。"
            )
            return
        ...
        if any(
            ...
            if missing_strict:
                print(
                    f"錯誤 (DBManager - insert_strategic_signals): DataFrame 缺少核心必要欄位: {', '.join(missing_strict)}。無法寫入 StrategicDashboard_Daily。"
                )
                df_to_insert.info()
                return
        ...
        for col in required_cols:
            if col not in df_to_insert.columns:
                df_to_insert[col] = (
                    "" if col == "commentary" else pd.NA
                )  # 或 np.nan for value if appropriate
        ...
        try:
            ...
            df_to_insert["signal"] = df_to_insert["signal"].astype(
                str
            )  # signal 通常是 'RED', 'GREEN', 'YELLOW'
            # value 允許是 NA (例如某些指標可能沒有數值，只有信號)
            df_to_insert["value"] = pd.to_numeric(
                df_to_insert["value"], errors="coerce"
            )  # coerce 會將無法轉換的變為 NaT/NaN
            df_to_insert["commentary"] = (
                df_to_insert["commentary"].astype(str).fillna("")
            )  # 確保是字串且 NaN 轉為空字串

        except Exception as e:
            print(
                f"錯誤 (DBManager - insert_strategic_signals): DataFrame 數據類型轉換失敗: {e}"
            )
            df_to_insert.info()
            return
        ...
        try:
            ...
            print(
                f"INFO (DBManager - insert_strategic_signals): 成功將 {num_rows} 筆戰略信號數據 ({unique_indicators} 個指標名) 寫入/更新至 '{table_name}'。"
            )

        except Exception as e:
            print(
                f"錯誤 (DBManager - insert_strategic_signals): 寫入戰略信號數據到資料表 '{table_name}' 失敗: {e}"
            )
            print("DEBUG: 嘗試寫入的戰略信號 DataFrame (前5行):")
            print(df_to_insert.head())
            df_to_insert.info()
...
if __name__ == "__main__":
    ...
    for min_offset in range(0, 60, 15):
        ts = day_for_minute_data_dt.replace(
            hour=9, minute=min_offset, second=0, microsecond=0, tzinfo=timezone.utc
        )
        multi_day_data.append(
            {
                "datetime": ts,
                "ticker": "AAPL",
                "interval": "15m",
                "open": 152.0 + min_offset * 0.01,
                "high": 152.5 + min_offset * 0.01,
                "low": 151.5 + min_offset * 0.01,
                "close": 152.2 + min_offset * 0.01,
                "volume": 5000 + min_offset * 10,
            }
        )
    ...
    if df_multi_day_all["datetime"].dt.tz is None:
        df_multi_day_all["datetime"] = df_multi_day_all["datetime"].dt.tz_localize(
            "UTC"
        )
    else:
    ...
    aapl_2024_07_22_data = db_manager.query_data_for_day(
        ticker="AAPL", date_str="2024-07-22", table_name=table_name
    )
    ...
    msft_2024_07_22_data = db_manager.query_data_for_day(
        ticker="MSFT", date_str="2024-07-22", table_name=table_name
    )
    ...
    prev_close_aapl = db_manager.query_previous_day_close(
        ticker="AAPL", current_date_str="2024-07-23", table_name=table_name
    )
    print(f"AAPL 前一日 (相對於 2024-07-23) 收盤價: {prev_close_aapl}")
    assert prev_close_aapl == 151.0 + 2
    prev_close_msft_weekend = db_manager.query_previous_day_close(
        ticker="MSFT", current_date_str="2024-07-21", table_name=table_name
    )
    print(f"MSFT 前一日 (相對於 2024-07-21) 收盤價: {prev_close_msft_weekend}")
    assert prev_close_msft_weekend == 201.0
    prev_close_way_back = db_manager.query_previous_day_close(
        ticker="AAPL", current_date_str="2024-07-19", table_name=table_name
    )
    ...
    assert (
        len(cached_df1) == 3
    ), f"案例 1 cached_df1 應有 3 筆數據, 實際: {len(cached_df1)}"
    ...
    print(
        f"案例 1: cached_df1 ({len(cached_df1)} rows), missing_dates1: {missing_dates1}"
    )
    ...
    assert (
        len(cached_df2) == 4
    ), f"案例 2 cached_df2 應有 4 筆數據, 實際: {len(cached_df2)}"
    ...
    print(
        f"案例 2: cached_df2 ({len(cached_df2)} rows), missing_dates2: {missing_dates2}"
    )

    print("\n案例 3: AAPL, '1d', 2024-07-23 to 2024-07-25 (部分命中)")
    cached_df3, missing_dates3 = db_manager.check_cache(
        ticker="AAPL",
        start_date_str="2024-07-23",
        end_date_str="2024-07-25",
        interval="1d",
        table_name=table_name,
    )
    ...
    assert (
        len(cached_df3) == 2
    ), f"案例 3 cached_df3 應有 2 筆數據, 實際: {len(cached_df3)}"
    assert missing_dates3 == [
        "2024-07-25"
    ], f"案例 3 missing_dates3 應為 ['2024-07-25'], 實際: {missing_dates3}"
    print(
        f"案例 3: cached_df3 ({len(cached_df3)} rows), missing_dates3: {missing_dates3}"
    )
    ...
    assert missing_dates4 == [
        "2024-07-20",
        "2024-07-21",
    ], f"案例 4 missing_dates4 應為 ['2024-07-20', '2024-07-21'], 實際: {missing_dates4}"
    print(
        f"案例 4: cached_df4 ({len(cached_df4)} rows), missing_dates4: {missing_dates4}"
    )
    ...
    assert missing_dates5 == [
        "2024-07-20",
        "2024-07-21",
    ], f"案例 5 missing_dates5 應為 ['2024-07-20', '2024-07-21'], 實際: {missing_dates5}"
    print(
        f"案例 5: cached_df5 ({len(cached_df5)} rows), missing_dates5: {missing_dates5}"
    )
    ...
    cached_df6, missing_dates6 = db_manager.check_cache(
        ticker="AAPL",
        start_date_str="2024-07-26",
        end_date_str="2024-07-27",
        interval="1d",
        table_name=table_name,
    )
    assert cached_df6.empty, f"案例 6 cached_df6 應為空, 實際: {len(cached_df6)} rows"
    assert missing_dates6 == [
        "2024-07-26",
        "2024-07-27",
    ], f"案例 6 missing_dates6 應為 ['2024-07-26', '2024-07-27'], 實際: {missing_dates6}"
    print(
        f"案例 6: cached_df6 ({len(cached_df6)} rows), missing_dates6: {missing_dates6}"
    )
    ...
    assert (
        len(cached_df7) == 2
    ), f"案例 7 cached_df7 應有 2 筆數據 (07-20, 07-21), 實際: {len(cached_df7)}"
    assert missing_dates7 == [
        "2024-07-19"
    ], f"案例 7 missing_dates7 應為 ['2024-07-19'], 實際: {missing_dates7}"
    print(
        f"案例 7: cached_df7 ({len(cached_df7)} rows), missing_dates7: {missing_dates7}"
    )
