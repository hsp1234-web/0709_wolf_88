...
class AnalysisEngine:
    ...
    def analyze_daily_ticker_data(
        self, ticker: str, date_str: str, table_name: str = "market_ohlcv_analyzer"
    ) -> dict:
        """
        分析單一標的在某一天的市場表現。
        ...
        if daily_data_df.empty:
            ...
            return {
                "status": "no_data",
                "message": f"無 {ticker} 在 {date_str} 的數據。",
            }
        ...
        # 獲取前一日收盤價
        prev_close_price = self.db_manager.query_previous_day_close(
            ticker, date_str, table_name
        )
        ...
        analysis_result = {
            ...
            "prev_close": (
                f"{prev_close_price:.2f}" if prev_close_price is not None else "N/A"
            ),
            "change_pct": (
                f"{price_change_pct:+.2f}%"
                if price_change_pct != float("inf")
                else "新生或極大變化"
            ),
            "high": f"{high_price:.2f}",
            "low": f"{low_price:.2f}",
            "range_pct": (
                f"{volatility_range_pct:.2f}%"
                if volatility_range_pct != float("inf")
                else "極大波動或從0開始"
            ),
        ...
        analysis_result["change_pct"] = (
            f"{numeric_change_pct:+.2f}%"
            if numeric_change_pct not in [float("inf"), float("-inf")]
            else (
                "新生或極大變化"
                if numeric_change_pct != 0
                else ("N/A" if prev_close_price is None else "+0.00%")
            )
        )
        analysis_result["range_pct"] = (
            f"{numeric_vol_range_pct:.2f}%"
            if numeric_vol_range_pct != float("inf")
            else (
                "極大波動或從0開始"
                if numeric_vol_range_pct != 0
                else ("0.00%" if low_price == 0 and high_price == 0 else "N/A")
            )
        )
...
if __name__ == "__main__":
    ...
    # current_dir = os.path.dirname(os.path.abspath(__file__)) # 移除
    # if current_dir not in sys.path: # 移除
    #     sys.path.append(current_dir) # 移除

    try:
        from .db_manager import DBManager  # 改為相對導入，更標準
    except ImportError:
        print(
        ...
        class FallbackMockDBManager: # Renamed to avoid conflict
            ...
            def query_previous_day_close(self, ticker, current_date_str, table_name):
                return None
    ...
    class MockDBManagerForEngineTest:
        def query_data_for_day(self, ticker, date_str, table_name="default_table"):
            ...
            if ticker == "AAPL" and date_str == "2024-07-25":
                data = {
                    ...
                    "low": [149.0, 150.0, 149.5],
                    "close": [151.5, 150.8, 150.9],
        ...
        def query_previous_day_close(
            self, ticker, current_date_str, table_name="default_table"
        ):
    ...
    class MockDBForInterpretation(MockDBManagerForEngineTest):
        ...
        def query_previous_day_close(
            ...
            if (
                ticker in ["STRONG_UP", "SHARP_DROP", "NARROW", "VOLATILE"]
                and current_date_str == "2024-07-26"
            ):
                return 100.0
            return super().query_previous_day_close(
                ticker, current_date_str, table_name
            )
    ...
    print("\n--- 測試市場解讀規則 ---")
    res_strong_up = engine_interpret.analyze_daily_ticker_data(
        "STRONG_UP", "2024-07-26"
    )
    print(f"STRONG_UP 解讀: {res_strong_up['interpretation']}")
    assert res_strong_up["interpretation"] == "強勢上漲，動能顯著。"

    res_sharp_drop = engine_interpret.analyze_daily_ticker_data(
        "SHARP_DROP", "2024-07-26"
    )
    ...
    assert (
        res_volatile["interpretation"] == "日內波動劇烈，風險加劇。"
    )  # change 0.5%, range 5.05% -> 波動優先
