...
def main():
    ...
    parser.add_argument(
        "--tickers",
        help="要分析的標的列表，以逗號分隔 (例如: AAPL,MSFT)。在純報告模式下非必需，但若提供則用於報告。",
    )
    parser.add_argument(
        "--start-date", help="數據分析/獲取的起始日期 (格式: YYYY-MM-DD)。"
    )
    parser.add_argument(
        "--end-date", help="數據分析/獲取的結束日期 (格式: YYYY-MM-DD)。"
    )

    # 流程控制參數
    parser.add_argument(
        "--data-only", action="store_true", help="僅執行數據獲取和存儲流程。"
    )
    parser.add_argument(
        "--report-only",
        action="store_true",
        help="僅執行報告生成流程 (需要已存在的數據)。",
    )
    parser.add_argument(
        "--report-start-date",
        help="報告生成的起始日期 (格式: YYYY-MM-DD)，配合 --report-only 使用。",
    )
    parser.add_argument(
        "--report-end-date",
        help="報告生成的結束日期 (格式: YYYY-MM-DD)，配合 --report-only 使用。",
    )

    # 資料庫與表格參數
    parser.add_argument(
        "--db-path",
        default="data_workspace/daily_market_analyzer.duckdb",
        help="主分析資料庫的完整路徑 (例如: data_workspace/daily_market_analysis.duckdb)。",
    )
    parser.add_argument(
        "--db-name",
        default="daily_market_analysis.duckdb",  # 實際已較少直接使用，db_path 更為主要
        help="主分析資料庫的檔案名稱 (參考用，主要以 db_path 為準)。",
    )
    parser.add_argument(
        "--cache-db-path",
        help="DuckDB 快取資料庫的最終存檔路徑 (目前 yfinance_client 直接使用主DB進行快取檢查)。",
    )  # 說明其當前用途
    parser.add_argument(
        "--table-name",
        default="market_ohlcv_data",
    ...
    parser.add_argument(
        "--gdrive-root",
        default="/content/drive/MyDrive/",  # Typical Colab GDrive mount
        help="Google Drive 的根路徑。",
    )
    parser.add_argument(
        "--project-path-local",
        default="/content/panoramic_market_analyzer/",  # As per plan
    ...
    if active_local_first_mode:
        if not gdrive_db_path:  # Check if original db_path was provided
            print(
                "錯誤 (Local-First): 啟用 --enable-local-first 時，必須提供 --db-path 作為 Google Drive 上的原始資料庫路徑。"
            )
            sys.exit(1)
        ...
        if not db_filename:
            print(
                f"錯誤 (Local-First): 無法從 --db-path '{gdrive_db_path}' 推斷資料庫檔案名稱。"
            )
            ...
            local_db_dir = os.path.join(
                args.project_path_local, "data_workspace", "databases_local"
            )
            local_db_path_instance = os.path.join(local_db_dir, db_filename)
            print(
                f"INFO (Local-First): Configured for local processing. Local DB will be: {local_db_path_instance}"
            )
    ...
    if active_local_first_mode and local_db_path_instance:
        final_db_path_used_by_dbmanager = (
            local_db_path_instance  # Tentative, might change if copy fails
        )
    ...
    def log_hardware_stats(stage_name: str):
        try:
            ...
            print(
                f"DEBUG (Hardware Stats): Logged for stage '{stage_name}'. CPU: {cpu_percent}%, RAM: {ram_percent}%"
            )
    ...
    try:
        ...
        if args.report_only:
            if not args.report_start_date or not args.report_end_date:
                print(
                    "錯誤：當使用 --report-only 時，必須提供 --report-start-date 和 --report-end-date。"
                )
                sys.exit(1)
            if not args.tickers:
            ...
            if not args.tickers or not args.start_date or not args.end_date:
                print(
                    "錯誤：在完整流程模式下，必須提供 --tickers, --start-date, 和 --end-date。"
                )
                sys.exit(1)
        elif args.data_only:  # Data only
            if not args.tickers or not args.start_date or not args.end_date:
                print(
                    "錯誤：當使用 --data-only 時，必須提供 --tickers, --start-date, 和 --end-date。"
                )
                sys.exit(1)
        ...
        try:
            ...
            print(
                f"INFO: 動態設定 DuckDB memory_limit 為: {memory_limit_setting_str} (基於 {available_gb:.2f} GB 可用記憶體)"
            )
        except Exception as e:
            print(
                f"警告: 偵測系統可用記憶體或設定 DuckDB 組態時發生錯誤: {e}。將使用 DuckDB 預設記憶體配置。"
            )
        ...
        if active_local_first_mode and local_db_path_instance and gdrive_db_path:
            print("INFO (Local-First): '本地優先' 模式已啟用。")
            ...
            if not os.path.exists(local_db_dir_for_copy):
                try:
                    ...
                    print(
                        f"INFO (Local-First): 已建立本地資料庫目錄: {local_db_dir_for_copy}"
                    )
                except Exception as e:
                    print(
                        f"錯誤 (Local-First): 建立本地資料庫目錄 '{local_db_dir_for_copy}' 失敗: {e}。停用本地優先模式。"
                    )
            ...
            if active_local_first_mode:
                if os.path.exists(gdrive_db_path):
                    try:
                        ...
                        print(
                            f"INFO (Local-First) [{copy_start_time.strftime('%H:%M:%S')}]: 開始複製資料庫從 GDrive '{gdrive_db_path}' 到本地 '{local_db_path_instance}'..."
                        )
                        total_size = os.path.getsize(gdrive_db_path)
                        print(
                            f"INFO (Local-First): 待複製檔案大小: {total_size / (1024*1024):.2f} MB."
                        )
                        ...
                        print(
                            f"INFO (Local-First) [{copy_end_time.strftime('%H:%M:%S')}]: 資料庫複製到本地完成。耗時: {duration_seconds:.2f} 秒。本地檔案大小: {copied_size / (1024*1024):.2f} MB."
                        )
                        final_db_path_used_by_dbmanager = local_db_path_instance
                    except Exception as e:
                        copy_fail_time = datetime.now()
                        print(
                            f"錯誤 (Local-First) [{copy_fail_time.strftime('%H:%M:%S')}]: 從 GDrive 複製資料庫失敗: {e}。停用本地優先模式。"
                        )
                        active_local_first_mode = False
                else:
                    print(
                        f"警告 (Local-First): GDrive 上的原始資料庫檔案 '{gdrive_db_path}' 不存在。將嘗試在本地路徑 '{local_db_path_instance}' 建立新資料庫。"
                    )
                    final_db_path_used_by_dbmanager = local_db_path_instance
        ...
        if not active_local_first_mode:
            final_db_path_used_by_dbmanager = gdrive_db_path
            print(
                f"INFO (Local-First Fallback): 本地優先模式已停用或初始化失敗。將使用資料庫: {final_db_path_used_by_dbmanager}"
            )

        print(
            f"INFO: 初始化 DBManager，資料庫路徑: {final_db_path_used_by_dbmanager}, 目標資料表: {args.table_name}"
        )
        ...
        if args.data_only:
            ...
            yf_client = YFinanceClient(
                ...
                no_data_cooldown_days=args.no_data_cooldown_days,
            )
            overall_execution_log, _ = run_data_pipeline(
                args, db_manager, yf_client, tickers_list
            )
            ...
            task_duration_seconds = (
                data_pipeline_end_time - data_pipeline_start_time
            ).total_seconds()
            log_hardware_stats("data_pipeline_end")
        ...
        if writer_thread is not None and writer_thread.is_alive():
            ...
            print(
                "INFO (Main): 向消費者線程發送停止事件信號 (stop_writer_event.set())。"
            )
            ...
            print(
                f"INFO (Main): 資料庫寫入消費者線程存在但非運行狀態 (is_alive: {writer_thread.is_alive()})。"
            )
        else:
        ...
        total_script_duration = (
            final_overall_end_time - overall_start_time
        ).total_seconds()
        print("\n--- 總任務執行完畢 ---")
        ...
        print(
            f"DEBUG (Finally): active_local_first_mode={active_local_first_mode}, local_db_path_instance={local_db_path_instance}"
        )
        if active_local_first_mode and local_db_path_instance and gdrive_db_path:
            if (
                db_manager
                and hasattr(db_manager, "_conn")
                and getattr(db_manager, "_conn", None) is not None
            ):
                print("INFO (Local-First): 嘗試關閉 DBManager 連線以釋放檔案鎖...")
                try:
                    del db_manager
                    print("INFO (Local-First): DBManager 實例已刪除。")
                except Exception as e:
            ...
            if os.path.exists(local_db_path_instance):
                try:
                    ...
                    print(
                        f"INFO (Local-First) [{writeback_start_time.strftime('%H:%M:%S')}]: 開始將本地資料庫 '{local_db_path_instance}' 回寫到 GDrive '{gdrive_db_path}'..."
                    )
                    gdrive_db_dir_for_writeback = os.path.dirname(gdrive_db_path)
                    if gdrive_db_dir_for_writeback and not os.path.exists(
                        gdrive_db_dir_for_writeback
                    ):
                        os.makedirs(gdrive_db_dir_for_writeback, exist_ok=True)
                        print(
                            f"INFO (Local-First): 已建立 GDrive 上的目標目錄 (回寫時): {gdrive_db_dir_for_writeback}"
                        )
                    local_size_for_wb = os.path.getsize(local_db_path_instance)
                    print(
                        f"INFO (Local-First): 待回寫本地檔案大小: {local_size_for_wb / (1024*1024):.2f} MB."
                    )
                    shutil.copy2(local_db_path_instance, gdrive_db_path)
                    writeback_end_time = datetime.now()
                    duration_seconds_wb = (
                        writeback_end_time - writeback_start_time
                    ).total_seconds()
                    print(
                        f"INFO (Local-First) [{writeback_end_time.strftime('%H:%M:%S')}]: 資料庫成功回寫到 GDrive '{gdrive_db_path}'。耗時: {duration_seconds_wb:.2f} 秒。"
                    )
                except Exception as e:
                    writeback_fail_time = datetime.now()
                    print(
                        f"錯誤 (Local-First) [{writeback_fail_time.strftime('%H:%M:%S')}]: 回寫資料庫到 GDrive 失敗: {e}"
                    )
            else:
                print(
                    f"警告 (Local-First): 本地資料庫檔案 '{local_db_path_instance}' 不存在於預期路徑，無法回寫。"
                )
        elif args.enable_local_first:
            print(
                "INFO (Local-First): 本地優先模式最初啟用，但可能在設置或執行過程中被停用/失敗。跳過回寫。"
            )
        if hardware_stats_list:
            run_timestamp_str = overall_start_time.strftime("%Y%m%d_%H%M%S")
            archive_hardware_stats_to_csv(
                hardware_stats_list, args, run_timestamp_str, gdrive_db_path
            )


def archive_hardware_stats_to_csv(
    stats_list: list,
    cli_args: argparse.Namespace,
    timestamp_str: str,
    base_gdrive_path_ref: str | None,
):
    if not stats_list:
        print("INFO (Hardware Archive): No hardware stats collected to archive.")
        return
    local_log_archive_base = os.path.join(
        cli_args.project_path_local, "data_workspace", "logs", "archive"
    )
    gdrive_log_archive_base = None
    if base_gdrive_path_ref:
        gdrive_project_data_workspace = os.path.dirname(base_gdrive_path_ref)
        if (
            os.path.basename(gdrive_project_data_workspace) == "databases_local"
            and "data_workspace" in gdrive_project_data_workspace
        ):
            gdrive_project_data_workspace = os.path.dirname(
                gdrive_project_data_workspace
            )
        if "data_workspace" in gdrive_project_data_workspace:
            gdrive_log_archive_base = os.path.join(
                gdrive_project_data_workspace, "logs", "archive"
            )
        else:
            gdrive_log_archive_base = os.path.join(
                cli_args.gdrive_root, "panoramic_market_analyzer_logs", "archive"
            )
            print(
                f"WARN (Hardware Archive): Could not determine GDrive project structure from '{base_gdrive_path_ref}'. Using fallback GDrive log path: {gdrive_log_archive_base}"
            )
    else:
        gdrive_log_archive_base = os.path.join(
            cli_args.gdrive_root, "panoramic_market_analyzer_logs", "archive"
        ...
        print(
            f"WARN (Hardware Archive): GDrive DB path not available. Using fallback GDrive log path: {gdrive_log_archive_base}"
        )
    ...
    try:
        ...
        print(
            f"INFO (Hardware Archive): Hardware stats successfully saved to local: {local_filepath}"
        )
    except Exception as e:
        print(
            f"錯誤 (Hardware Archive): 儲存硬體狀態到本地 '{local_filepath}' 失敗: {e}"
        )
    if local_filepath != gdrive_filepath:
        try:
            ...
            with open(gdrive_filepath, "w", newline="", encoding="utf-8") as f_gdrive:
                ...
                writer.writerows(stats_list)
            print(
                f"INFO (Hardware Archive): Hardware stats successfully saved to GDrive: {gdrive_filepath}"
            )
        except Exception as e:
            print(
                f"錯誤 (Hardware Archive): 儲存硬體狀態到 GDrive '{gdrive_filepath}' 失敗: {e}"
            )


def run_data_pipeline(
    args, db_manager: DBManager, yf_client: YFinanceClient, tickers_list: list
):
    print("\n--- 開始數據處理流程 ---")
    ...
    print(
        f"INFO (run_data_pipeline): 開始並行獲取 {len(tickers_list)} 個標的的市場數據，使用最多 {args.max_workers} 個執行緒。"
    )
    future_to_ticker = {}
    with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
        ...
        for future in as_completed(future_to_ticker):
            ...
            try:
                ...
                if ticker_execution_log:
                    for (
                        date_key,
                        daily_log_for_all_tickers_on_date,
                    ) in ticker_execution_log.items():
                        if date_key not in current_overall_execution_log:
                            current_overall_execution_log[date_key] = {}
                        current_overall_execution_log[date_key].update(
                            daily_log_for_all_tickers_on_date
                        )
                if ticker_execution_log:
                    ...
                    for date_log in ticker_execution_log.values():
                        if ticker_symbol_for_log in date_log:
                            final_status_messages.append(
                                date_log[ticker_symbol_for_log].get("message", "無訊息")
                            )
                    if any(
                        "queued" in msg.lower() or "cached" in msg.lower()
                        for msg in final_status_messages
                    ):
                        log_status_for_ticker = "已處理 (數據已入隊或來自快取)"
                    elif final_status_messages:
                        log_status_for_ticker = (
                            f"已處理 (最終日誌: {final_status_messages[-1][:100]})"
                        )
                    else:
                        log_status_for_ticker = "已分派處理，但未找到明確的最終狀態日誌"
                    print(
                        f"INFO (Parallel Result): 標的 {ticker_symbol_for_log} 並行生產者任務完成。狀態: {log_status_for_ticker}。日誌已合併。"
                    )
                else:
                    print(
                        f"警告 (Parallel Result): 標的 {ticker_symbol_for_log} 並行處理未返回日誌。"
                    )
            except Exception as exc:
                print(
                    f"錯誤 (Parallel Task): 標的 {ticker_symbol_for_log} 的並行任務執行失敗: {exc}"
                )
                for date_str_key_for_error in pd.date_range(
        ...
        print("INFO (run_data_pipeline): 所有並行標的數據獲取任務已提交到執行緒池。")
...
def run_report_generation(
    ...
    input_execution_log: dict,
    report_tickers_list: list,
    report_overall_start_time: datetime,
    data_task_duration_seconds: float,
    ...
    if not report_tickers_list:
        ...
        return
    run_timestamp_str_for_hw_report = report_overall_start_time.strftime(
        "%Y%m%d_%H%M%S"
    )
    hw_report_filename = (
        f"hardware_monitor_report_{run_timestamp_str_for_hw_report}.csv"
    )
    local_log_archive_base_for_ref = os.path.join(
        args.project_path_local, "data_workspace", "logs", "archive"
    )
    hardware_report_csv_path_ref = os.path.join(
        local_log_archive_base_for_ref, hw_report_filename
    )
    ...
    print(
        f"INFO (run_report_generation): Generating report for tickers: {report_tickers_list} over range [{report_start_d} to {report_end_d}]"
    )
    ...
    report_duration_seconds = (
        report_pipeline_end_time - report_pipeline_start_time
    ).total_seconds()
...
def database_writer_worker(
    ...
    print(
        f"INFO (DB Writer Thread): 消費者線程 '{threading.current_thread().name}' 已啟動。"
    )
    while not stop_event.is_set() or not data_q.empty():
        try:
            ...
            if data_item is not None:
                ...
                item_ticker = (
                    data_item.get("ticker", "N/A")
                    if isinstance(data_item, dict)
                    else "N/A (item not dict)"
                )
                item_interval = (
                    data_item.get("interval", "N/A")
                    if isinstance(data_item, dict)
                    else "N/A (item not dict)"
                )
                print(
                    f"DEBUG (DB Writer Thread): 從佇列收到項目: Ticker: {item_ticker}, Interval: {item_interval}, Data: {df_summary}"
                )
            elif data_item is None:
                print(
                    "DEBUG (DB Writer Thread): 從佇列收到項目: Termination Signal (None)"
                )
        except queue.Empty:
            ...
            with lock:
                ...
                if df_to_write is not None and not df_to_write.empty and table_to_write:
                    try:
                        ...
                        print(
                            f"INFO (DB Writer Thread): 成功寫入 {len(df_to_write)} 筆來自 '{ticker_symbol}' (顆粒度: {interval_val}) 的數據到資料表 '{table_to_write}'。"
                        )
                    except duckdb.ConstraintException as ce:
                        print(
                            f"INFO (DB Writer Thread): 數據已存在或違反唯一約束，跳過寫入 Ticker: {ticker_symbol} (顆粒度: {interval_val}) 到 {table_to_write}。錯誤: {ce}"
                        )
                    except Exception as e_upsert:
                        print(
                            f"ERROR (DB Writer Thread): 寫入數據時發生未預期錯誤 Ticker: {ticker_symbol} (顆粒度: {interval_val}) 到 {table_to_write}。錯誤: {e_upsert}"
                        )
                else:
                    print(
                        f"WARN (DB Writer Thread): 從佇列收到的數據項無效或不完整: Ticker {ticker_symbol}, Table {table_to_write}, DataFrame is None or empty: {df_to_write is None or df_to_write.empty}"
                    )
        except Exception as e:
            print(
                f"ERROR (DB Writer Thread): 消費者線程處理項目時發生嚴重錯誤: {e}. Data item (部分): {{'ticker': {data_item.get('ticker','N/A') if data_item else 'N/A'}, 'table': {data_item.get('table_name','N/A') if data_item else 'N/A'}}}"
            )
        finally:
            data_q.task_done()
    print(
        f"INFO (DB Writer Thread): 消費者線程 '{threading.current_thread().name}' 已結束。"
    )
